...
eReturnValues seagate_ata_SCT_SATA_phy_speed(const tDevice* device, uint8_t speedGen)
{
    ...
    // option flags
    sctSATAPhySpeed[8] = 0x01; // save feature to disk...I'm assuming this is always what's wanted since the other flags
                               // don't make sense since this feature requires a power cycle
...
typedef enum eSASPhySpeedsEnum
{
...
eReturnValues scsi_Set_Phy_Speed(const tDevice* device, uint8_t phySpeedGen, bool allPhys, uint8_t phyNumber)
{
    ...
    uint16_t phyControlLength =
        UINT16_C(104) +
        MODE_PARAMETER_HEADER_10_LEN; // size of 104 comes from 8 byte page header + (2 * 48bytes) for 2 phy descriptors
                                      // + then add 8 bytes for mode parameter header. This is assuming drives only
                                      // have 2...which is true right now, but the code will detect when it needs to
    ...
    if (SUCCESS == scsi_Mode_Sense_10(device, MP_PROTOCOL_SPECIFIC_PORT, phyControlLength, 0x01, true, false,
                                      MPC_CURRENT_VALUES, sasPhyControl))
    {
        ...
        if ((sasPhyControl[MODE_PARAMETER_HEADER_10_LEN + blockDescriptorLength + 0] & 0x3F) == 0x19 &&
            ...
            (sasPhyControl[MODE_PARAMETER_HEADER_10_LEN + blockDescriptorLength + 0] & BIT6) > 0)
        {
            uint16_t pageLength =
                M_BytesTo2ByteValue(sasPhyControl[MODE_PARAMETER_HEADER_10_LEN + blockDescriptorLength + 2],
                                    sasPhyControl[MODE_PARAMETER_HEADER_10_LEN + blockDescriptorLength + 3]) +
                4; // add 4 for first 4 bytes of page before descriptors
            // check that we were able to read the full page! If we didn't get the entire thing, we need to reread it
            // and adjust the phyControlLength variable!
            if ((pageLength + MODE_PARAMETER_HEADER_10_LEN + blockDescriptorLength) > phyControlLength)
            {
                ...
                sasPhyControl = temp;
                if (SUCCESS != scsi_Mode_Sense_10(device, MP_PROTOCOL_SPECIFIC_PORT, phyControlLength, 0x01, true,
                                                  false, MPC_CURRENT_VALUES, sasPhyControl))
                {
                    safe_free_aligned(&sasPhyControl);
                    return FAILURE;
                }
            }
            if ((sasPhyControl[MODE_PARAMETER_HEADER_10_LEN + blockDescriptorLength + 5] & 0x0F) ==
                6) // make sure it's the SAS protocol page
            {
                uint8_t  numberOfPhys        = sasPhyControl[MODE_PARAMETER_HEADER_10_LEN + blockDescriptorLength + 7];
                uint32_t phyDescriptorOffset = MODE_PARAMETER_HEADER_10_LEN + blockDescriptorLength +
                                               8; // this will be set to the beginning of the phy descriptors so that
                                                  // when looping through them, it is easier code to read.
                for (uint16_t phyIter = UINT16_C(0);
                     phyIter < C_CAST(uint16_t, numberOfPhys) && phyDescriptorOffset < phyControlLength;
                     ++phyIter, phyDescriptorOffset += 48)
                {
                    uint8_t phyIdentifier = sasPhyControl[phyDescriptorOffset + 1];
                    // check if the caller requested changing all phys or a specific phy and only modify it's descriptor
                    ...
                    if (allPhys || phyNumber == phyIdentifier)
                    {
                        ...
                        if (phySpeedGen == 0)
                        {
                            // they want it back to default, so read the hardware maximum physical link rate and set it
                            // to the programmed maximum
                            uint8_t matchedRate =
                                C_CAST(uint8_t, (hardwareMaximumLinkRate << 4) | hardwareMaximumLinkRate);
                        ...
                        else
                        {
                            ...
                            {
                            case 1: // 1.5 Gb/s
                                sasPhyControl[phyDescriptorOffset + 33] =
                                    (SAS_PHY_1_5_Gbs << 4) | hardwareMaximumLinkRate;
                                break;
                            case 2: // 3.0 Gb/s
                                sasPhyControl[phyDescriptorOffset + 33] =
                                    (SAS_PHY_3_Gbs << 4) | hardwareMaximumLinkRate;
                                break;
                            case 3: // 6.0 Gb/s
                                sasPhyControl[phyDescriptorOffset + 33] =
                                    (SAS_PHY_6_Gbs << 4) | hardwareMaximumLinkRate;
                                break;
                            case 4: // 12.0 Gb/s
                                sasPhyControl[phyDescriptorOffset + 33] =
                                    (SAS_PHY_12_Gbs << 4) | hardwareMaximumLinkRate;
                                break;
                            case 5: // 22.5 Gb/s
                                sasPhyControl[phyDescriptorOffset + 33] =
                                    (SAS_PHY_22_5_Gbs << 4) | hardwareMaximumLinkRate;
                                break;
                            default:
                ...
                if (SUCCESS !=
                    scsi_Mode_Select_10(device, phyControlLength, true, true, false, sasPhyControl, phyControlLength))
                {
                    ret = FAILURE;
                }
            }
    ...
    if (device->drive_info.drive_type == ATA_DRIVE)
    {
        if (is_Seagate_Family(device) == SEAGATE)
        {
            if (is_ATA_Identify_Word_Valid(le16_to_host(device->drive_info.IdentifyData.ata.Word206)) &&
                le16_to_host(device->drive_info.IdentifyData.ata.Word206) & BIT7 && !is_SSD(device))
            {
...
bool is_SCT_Low_Current_Spinup_Supported(const tDevice* device)
{
    ...
    if (device->drive_info.drive_type == ATA_DRIVE && is_Seagate_Family(device) == SEAGATE)
    {
        ...
        {
            uint16_t optionFlags = UINT16_C(0x0000);
            uint16_t state       = UINT16_C(0x0000);
            if (SUCCESS == send_ATA_SCT_Feature_Control(device, SCT_FEATURE_FUNCTION_RETURN_CURRENT_STATE,
                                                        SEAGATE_SCT_FEATURE_CONTROL_LOW_CURRENT_SPINUP, &state,
                                                        &optionFlags))
            {
                supported = true;
            }
...
int is_Low_Current_Spin_Up_Enabled(const tDevice* device, bool sctCommandSupported)
{
    ...
    if (device->drive_info.drive_type == ATA_DRIVE && is_Seagate_Family(device) == SEAGATE)
    {
        ...
        if (is_ATA_Identify_Word_Valid(le16_to_host(device->drive_info.IdentifyData.ata.Word206)) &&
            (le16_to_host(device->drive_info.IdentifyData.ata.Word206) & BIT4) && sctCommandSupported)
        {
            uint16_t optionFlags = UINT16_C(0x0000);
            uint16_t state       = UINT16_C(0x0000);
            if (SUCCESS == send_ATA_SCT_Feature_Control(device, SCT_FEATURE_FUNCTION_RETURN_CURRENT_STATE,
        ...
        else if (!sctCommandSupported) // check the identify data for a bit (2.5" drives only I think) - TJE
        {
            ...
            if (is_ATA_Identify_Word_Valid(le16_to_host(device->drive_info.IdentifyData.ata.Word155)) &&
                le16_to_host(device->drive_info.IdentifyData.ata.Word155) & BIT1)
            {
...
eReturnValues seagate_SCT_Low_Current_Spinup(const tDevice* device, eSeagateLCSpinLevel spinupLevel)
{
    ...
    if (is_ATA_Identify_Word_Valid(le16_to_host(device->drive_info.IdentifyData.ata.Word206)) &&
        le16_to_host(device->drive_info.IdentifyData.ata.Word206) & BIT4)
    {
        uint16_t saveToDrive =
            UINT16_C(0x0001); // always set this because this feature requires saving for it to even work.
        uint16_t state = C_CAST(uint16_t, spinupLevel);
        if (SUCCESS == send_ATA_SCT_Feature_Control(device, SCT_FEATURE_FUNCTION_SET_STATE_AND_OPTIONS,
                                                    SEAGATE_SCT_FEATURE_CONTROL_LOW_CURRENT_SPINUP, &state,
                                                    &saveToDrive))
        {
            ret = SUCCESS;
...
eReturnValues set_Low_Current_Spin_Up(const tDevice* device, bool useSCTCommand, eSeagateLCSpinLevel state)
{
    ...
    if (device->drive_info.drive_type == ATA_DRIVE && is_Seagate_Family(device) == SEAGATE)
    {
        ...
        else
        {
            ...
            if (state == SEAGATE_LOW_CURRENT_SPINUP_STATE_DEFAULT) // 0 means disable, 2 is here for compatibility with
                                                                   // SCT command inputs
            {
            ...
            if (SUCCESS == ata_Set_Features(device, C_CAST(eATASetFeaturesSubcommands, SEAGATE_SF_LOW_CURRENT_SPINUP),
                                            0, secCnt, LOW_CURRENT_SPINUP_LBA_MID_SIG, LOW_CURRENT_SPINUP_LBA_HI_SIG))
            {
                ret = SUCCESS;
            }
        }
    ...
    if (device->drive_info.drive_type == ATA_DRIVE)
    {
        if (is_Seagate_Family(device) == SEAGATE)
        {
                ...
                le16_to_host(device->drive_info.IdentifyData.ata.Word206) & BIT4)
            {
                ...
                uint16_t saveToDrive = UINT16_C(0x0001);
                if (SUCCESS == send_ATA_SCT_Feature_Control(device, SCT_FEATURE_FUNCTION_SET_STATE_AND_OPTIONS,
                                                            SEAGATE_SCT_FEATURE_CONTROL_SPEAD_SPECTRUM_CLOCKING, &state,
                                                            &saveToDrive))
                {
                    ret = SUCCESS;
                }
    ...
    if (device->drive_info.drive_type == ATA_DRIVE)
    {
        if (is_Seagate_Family(device) == SEAGATE)
        {
                ...
                le16_to_host(device->drive_info.IdentifyData.ata.Word206) & BIT4)
            {
                ...
                uint16_t saveToDrive = UINT16_C(0);
                if (SUCCESS == send_ATA_SCT_Feature_Control(device, SCT_FEATURE_FUNCTION_RETURN_CURRENT_STATE,
                                                            SEAGATE_SCT_FEATURE_CONTROL_SPEAD_SPECTRUM_CLOCKING, &state,
                                                            &saveToDrive))
                {
...
static eReturnValues seagate_SAS_Get_JIT_Modes(const tDevice* device, ptrSeagateJITModes jitModes)
{
    ...
    if (family == SEAGATE || family == SEAGATE_VENDOR_A)
    {
        if (!is_SSD(device))
        {
            ...
            if (SUCCESS == scsi_Mode_Sense_10(device, 0, 12 + MODE_PARAMETER_HEADER_10_LEN, 0, true, false,
                                              MPC_CURRENT_VALUES, seagateUnitAttentionParameters))
            {
                readPage = true;
            }
            else if (SUCCESS == scsi_Mode_Sense_6(device, 0, 12 + MODE_PARAMETER_HEADER_6_LEN, 0, true,
                                                  MPC_CURRENT_VALUES, seagateUnitAttentionParameters))
            {
                readPage     = true;
                                               ...
                                               bool           nonvolatile)
{
    ...
    if (family == SEAGATE || family == SEAGATE_VENDOR_A)
    {
        if (!is_SSD(device))
        {
            ...
            if (revertToDefaults)
            {
                // We need to read the default mode page to get the status of the JIT bits, save them, then pass them
                // along...
                bool readDefaults = false;
                if (SUCCESS == scsi_Mode_Sense_10(device, 0, 12 + MODE_PARAMETER_HEADER_10_LEN, 0, true, false,
                                                  MPC_DEFAULT_VALUES, seagateUnitAttentionParameters))
                {
                    readDefaults = true;
            ...
            if (readPage)
            {
                ...
                seagateUnitAttentionParameters[headerLength + 4] &=
                    0x70; // clear all bits to zero, except 4, 5, & 6 since those are reserved (and if they are ever
                          // used, we don't want to touch them now) - TJE
                if (!disableVjit)
                {
                    seagateUnitAttentionParameters[headerLength + 4] |= BIT7;
                }
                switch (jitMode) // Spec says that faster modes allow the drive to continue accessing slower modes, so
                                 // might as well set each bit below the requested mode.
                {
                default:
                case 0:
                ...
                {
                    ret = scsi_Mode_Select_10(device, 12 + headerLength, false, nonvolatile, false,
                                              seagateUnitAttentionParameters, 12 + headerLength);
                }
                else
                {
                    ret = scsi_Mode_Select_6(device, 12 + headerLength, false, nonvolatile, false,
                                             seagateUnitAttentionParameters, 12 + headerLength);
                }
            }
            else
...
eReturnValues seagate_Get_Power_Balance(const tDevice* device, bool* supported, bool* enabled)
{
    ...
    if (device->drive_info.drive_type == ATA_DRIVE)
    {
        if (is_Seagate_Family(device) == SEAGATE)
        {
            ...
            if (supported != M_NULLPTR)
            {
                ...
                if (is_ATA_Identify_Word_Valid(le16_to_host(device->drive_info.IdentifyData.ata.Word149)) &&
                    (le16_to_host(device->drive_info.IdentifyData.ata.Word149) & BIT8 ||
                     le16_to_host(device->drive_info.IdentifyData.ata.Word149) & BIT10))
                {
                    *supported = true;
                }
                else
            ...
            if (enabled != M_NULLPTR)
            {
                ...
                if (is_ATA_Identify_Word_Valid(le16_to_host(device->drive_info.IdentifyData.ata.Word149)) &&
                    (le16_to_host(device->drive_info.IdentifyData.ata.Word149) & BIT9 ||
                     le16_to_host(device->drive_info.IdentifyData.ata.Word149) & BIT11))
                {
                    *enabled = true;
                }
                else
    ...
    else if (device->drive_info.drive_type == SCSI_DRIVE)
    {
        ...
        //       New drives use the active power mode identifier and the "highest" means disabled and "lowest" means
        //       enabled for this feature - TJE
        if (is_Seagate_Family(device) == SEAGATE)
        {
            uint32_t powerConsumptionLength = UINT32_C(0);
                                              ...
                                              true, false, MPC_CHANGABLE_VALUES, pcModePage))
            {
                         ...
                         pcModePage[MODE_PARAMETER_HEADER_10_LEN + 7] == 0)
                {
                                                      ...
                                                      0x01, true, false, MPC_CURRENT_VALUES, pcModePage))
                    {
                        ...
                        else if (activeLevel == 1 && pcModePage[MODE_PARAMETER_HEADER_10_LEN + 7] == 0 &&
                                 enabled != M_NULLPTR)
                        {
                            *enabled = false;
                        }
...
eReturnValues seagate_Set_Power_Balance(const tDevice* device, ePowerBalanceMode powerMode)
{
    ...
    else if (device->drive_info.drive_type == SCSI_DRIVE)
    {
        ...
        // First, need to read changable values page to see if this is a drive needing the old method, or the new one. -
        // TJE
        if (SUCCESS == scsi_Mode_Sense_10(device, MP_POWER_CONSUMPTION, MODE_PARAMETER_HEADER_10_LEN + 16, 0x01, true,
                                          false, MPC_CHANGABLE_VALUES, pcModePage))
        {
        ...
        if (SUCCESS == scsi_Mode_Sense_10(device, MP_POWER_CONSUMPTION, 16 + MODE_PARAMETER_HEADER_10_LEN, 0x01, true,
                                          false, MPC_CURRENT_VALUES, pcModePage))
        {
            ...
            ret = scsi_Mode_Select_10(device, 16 + MODE_PARAMETER_HEADER_10_LEN, true, true, false, pcModePage,
                                      16 + MODE_PARAMETER_HEADER_10_LEN);
        }
        safe_free_aligned(&pcModePage);
    }
    return ret;
        ...
        if (is_Seagate_Family(device) == SEAGATE)
        {
            ...
            case SEAGATE_IDD_LONG:
                ...
                if (ret == SUCCESS)
                {
                    ...
                    {
                        numberOfLbasInLists +=
                            M_BytesTo4ByteValue(smartData.attributes.ataSMARTAttr.attributes[5].data.rawData[3],
        ...
        if (iddDiagPage != M_NULLPTR)
        {
            ...
            if (ret != SUCCESS)
            {
                ...
                get_Sense_Key_ASC_ASCQ_FRU(device->drive_info.lastCommandSenseData, SPC3_SENSE_LEN, &senseKey, &asc,
                                           &ascq, &fru);
                // We are checking if the reset part of the test is still in progress by checking for this sense data
                // and will try again after a second until we know this part of the test is complete.
                if (senseKey == SENSE_KEY_UNIT_ATTENTION && asc == 0x29 && ascq == 0x01 && fru == 0x0A)
                {
                    *status = 0x0F; // this is setting that the IDD is still in progress. This is because we got
                                    // specific sense data related to this test, so that is why we are setting this here
                                    // instead of trying to look at the data buffer.
                    return SUCCESS;
                }
            }
            if (iddDiagPage[0] == SEAGATE_DIAG_IN_DRIVE_DIAGNOSTICS &&
                M_BytesTo2ByteValue(iddDiagPage[2], iddDiagPage[3]) ==
                    0x0008) // check that the page and pagelength match what we expect
            {
                ret     = SUCCESS;
...
static eReturnValues start_IDD_Operation(const tDevice* device, eIDDTests iddOperation, bool captiveForeground)
{
    ...
    if (device->drive_info.drive_type == ATA_DRIVE)
    {
        uint8_t  iddTestNumber  = UINT8_C(0);
    ...
    uint32_t commandTimeSeconds =
        C_CAST(uint32_t, device->drive_info.lastCommandTimeNanoSeconds / UINT64_C(1000000000));
    if (commandTimeSeconds < IDD_READY_TIME_SECONDS)
    {
        // we need to make sure we waited at least 2 minutes since command was sent to the drive before pinging it with
        // another command. It needs time to spin back up and be ready to accept commands again. This is being done in
        // both captive/foreground and offline/background modes due to differences between some drive firmwares.
...
eReturnValues run_IDD(const tDevice* device, eIDDTests IDDtest, bool pollForProgress, bool captive)
{
    ...
    if (is_Seagate_Family(device) != NON_SEAGATE)
    {
        ...
        if (SUCCESS == get_IDD_Support(device, &iddSupport))
        {
            ...
            if ((IDDtest == SEAGATE_IDD_SHORT && iddSupport.iddShort) ||
                (IDDtest == SEAGATE_IDD_LONG && iddSupport.iddLong))
            {
                uint8_t status            = UINT8_C(0xF);
                bool    captiveForeground = false;
                if (IDDtest == SEAGATE_IDD_SHORT || captive)
                {
                    // SCSI says that the short test must be run in foreground...so let's do that for both ATA and
                    // SCSI... Long test may be ran in background or foreground
                ...
                if (SUCCESS == result && captiveForeground)
                {
                    ...
                    else
                    {
                        ...
                        default:
                            ...
                            {
                                // IDD may still be in progress, so start polling for progress just to make sure it
                                // finished. This is a special case for the long test since it can take more than 2
                                // minutes to complete.
                                pollForProgress = true;
                            }
                            break;
...
bool is_Seagate_Power_Telemetry_Feature_Supported(const tDevice* device)
{
    ...
    else if (device->drive_info.drive_type == SCSI_DRIVE)
    {
        ...
        if (SUCCESS == get_SCSI_Error_History_Size(device, SEAGATE_ERR_HIST_POWER_TELEMETRY, &bufferSize, false,
                                                   is_SCSI_Read_Buffer_16_Supported(device)))
        {
            if (bufferSize > 0)
            {
                supported = true;
...
eReturnValues pull_Power_Telemetry_Log(const tDevice* device, const char* filePath, uint32_t transferSizeBytes)
{
    ...
    if (device->drive_info.drive_type == ATA_DRIVE)
    {
        ret = get_ATA_Log(device, SEAGATE_ATA_LOG_POWER_TELEMETRY, "PWRTEL", "pwr", true, false, false, M_NULLPTR, 0,
                          filePath, transferSizeBytes, 0);
    }
    else if (device->drive_info.drive_type == SCSI_DRIVE)
    {
        ret = get_SCSI_Error_History(device, SEAGATE_ERR_HIST_POWER_TELEMETRY, "PWRTEL", false,
                                     is_SCSI_Read_Buffer_16_Supported(device), "pwr", false, M_NULLPTR, 0, filePath,
                                     transferSizeBytes, M_NULLPTR);
    }
    return ret;
}
                                        ...
                                        ePowerTelemetryMeasurementOptions measurementOption)
{
    ...
    if (device->drive_info.drive_type == ATA_DRIVE)
    {
        ...
        // send write log ext
        ret = ata_Write_Log_Ext(device, SEAGATE_ATA_LOG_POWER_TELEMETRY, 0, pwrTelLogPg, 512,
...
eReturnValues get_Power_Telemetry_Data(const tDevice* device, ptrSeagatePwrTelemetry pwrTelData)
{
    ...
    else if (device->drive_info.drive_type == SCSI_DRIVE)
    {
        ...
        ret =
            get_SCSI_Error_History_Size(device, SEAGATE_ERR_HIST_POWER_TELEMETRY, &powerTelemetryLogSize, false, rb16);
        if (ret == SUCCESS && powerTelemetryLogSize > 0)
        {
            powerTelemetryLog = M_REINTERPRET_CAST(uint8_t*, safe_calloc_aligned(powerTelemetryLogSize, sizeof(uint8_t),
                                                                                 device->os_info.minimumAlignment));
            if (powerTelemetryLog != M_NULLPTR)
            {
                ret =
                    get_SCSI_Error_History(device, SEAGATE_ERR_HIST_POWER_TELEMETRY, M_NULLPTR, false, rb16, M_NULLPTR,
                                           true, powerTelemetryLog, powerTelemetryLogSize, M_NULLPTR, 0, M_NULLPTR);
            }
            else
            {
                ret = MEMORY_FAILURE;
    ...
    if (ret == SUCCESS && powerTelemetryLogSize > 0)
    {
        ...
        // easy by other users
        pwrTelData->driveTimeStampForHostRequestedMeasurement =
            M_BytesTo8ByteValue(0, 0, powerTelemetryLog[15], powerTelemetryLog[14], powerTelemetryLog[13],
                                powerTelemetryLog[12], powerTelemetryLog[11], powerTelemetryLog[10]);
        pwrTelData->driveTimeStampWhenTheLogWasRetrieved =
            M_BytesTo8ByteValue(0, 0, powerTelemetryLog[21], powerTelemetryLog[20], powerTelemetryLog[19],
        ...
        pwrTelData->measurementWindowTimeMilliseconds =
            M_BytesTo2ByteValue(powerTelemetryLog[43], powerTelemetryLog[42]);

        pwrTelData->multipleLogicalUnits = (device->drive_info.numberOfLUs > 1) ? true : false;

        for (uint16_t measurementNumber = UINT16_C(0);
             measurementNumber < pwrTelData->numberOfMeasurements && measurementOffset < dataLength &&
             measurementOffset < powerTelemetryLogSize;
             ++measurementNumber, measurementOffset += 6)
        {
            pwrTelData->measurement[measurementNumber].fiveVoltMilliWatts =
                M_BytesTo2ByteValue(powerTelemetryLog[measurementOffset + 1], powerTelemetryLog[measurementOffset + 0]);
            pwrTelData->measurement[measurementNumber].twelveVoltMilliWatts =
                M_BytesTo2ByteValue(powerTelemetryLog[measurementOffset + 3], powerTelemetryLog[measurementOffset + 2]);
            pwrTelData->measurement[measurementNumber].reserved =
                M_BytesTo2ByteValue(powerTelemetryLog[measurementOffset + 5], powerTelemetryLog[measurementOffset + 4]);
            ...
            {
                // invalid or empty entry should only happen once at the end! loop conditions should also protect from
                // this!
                break;
            }
        }
...
void show_Power_Telemetry_Data(ptrSeagatePwrTelemetry pwrTelData)
{
    ...
    if (pwrTelData != M_NULLPTR)
    {
        ...
        double sum5v    = 0.0;
        double sum12v   = 0.0;
        double min5v    = DBL_MAX;
        double max5v    = DBL_MIN;
        double min12v   = DBL_MAX;
        ...
        print_str("    #\t  Time (sec)\t  5V Pwr (W)\t  12V Pwr (W)\t  Total (W)\n");
...
bool is_Seagate_Quick_Format_Supported(const tDevice* device)
{
    ...
    if (device->drive_info.drive_type == ATA_DRIVE) // This is only available on SATA drives.
    {
        ...
        if (family == SEAGATE)
        {
            ...
            if (!supported)
            {
                // This above support bit was discontinued, so need to check for support of other features..This is not
                // a guaranteed "it will work" but it is what we have to work with.
...
uint64_t smart_attribute_vs(uint16_t verNo, SmartVendorSpecific attr)
{
    ...
    {
        attrFb = M_REINTERPRET_CAST(fb_smart_attribute_data*, &attr);
        val    = le32_to_host(attrFb->MSDword);
        val    = (val << 32) | le32_to_host(attrFb->LSDword);
        return val;
    }
    ...
    /******************************************************************
        if(attr.AttributeNumber == VS_ATTR_POWER_CONSUMPTION) {
            attrFb = M_REINTERPRET_CAST(fb_smart_attribute_data*, &attr);
            return le32_to_host(attrFb->LSDword);
        }
            ...
            attrFb = M_REINTERPRET_CAST(fb_smart_attribute_data*, &attr);
            val = le32_to_host(attrFb->MSDword);
            val = (val << 32) | ale32_to_host(ttrFb->LSDword );
            return val;
        }
...
{
    static uint64_t lsbGbErased      = UINT64_C(0);
    static uint64_t msbGbErased      = UINT64_C(0);
    static uint64_t lsbLifWrtToFlash = UINT64_C(0);
    static uint64_t msbLifWrtToFlash = UINT64_C(0);
    static uint64_t lsbLifWrtFrmHost = UINT64_C(0);
    static uint64_t msbLifWrtFrmHost = UINT64_C(0);
    static uint64_t lsbLifRdToHost   = UINT64_C(0);
    static uint64_t msbLifRdToHost   = UINT64_C(0);
    static uint64_t lsbTrimCnt       = UINT64_C(0);
    static uint64_t msbTrimCnt       = UINT64_C(0);
...
typedef enum eATAMaxSupportLogEntriesEnum
{
...
bool is_Seagate_DeviceStatistics_Supported(const tDevice* device)
{
    ...
    // Disable clang-tidy for readability
    if ((device->drive_info.drive_type == ATA_DRIVE) &&
        (get_ATA_Log_Size(device, 0xC7, &logSize, true, false) == SUCCESS))
    {
        supported = true;
    }
    else if ((device->drive_info.drive_type == SCSI_DRIVE) &&
             (get_SCSI_Log_Size(device, 0x2F, 0x00, &logSize) == SUCCESS))
    {
        supported = true;
    }
#if defined(_DEBUG)
                                                      ...
                                                      ptrSeagateDeviceStatistics seagateDeviceStats)
{
    ...
    if (SUCCESS == get_ATA_Log_Size(device, 0xC7, &deviceStatsSize, true, false))
    {
        ...
        if (SUCCESS == get_ATA_Log(device, 0xC7, M_NULLPTR, M_NULLPTR, true, false, true, deviceStatsLog,
                                   deviceStatsSize, M_NULLPTR, 0, 0))
        {
            ret                              = SUCCESS;
            uint8_t   maxLogEntries          = UINT8_C(0);
            ...
            for (uint8_t logEntry = UINT8_C(0); logEntry < maxLogEntries; ++logEntry)
            {
                ...
                case 0:
                    ...
                    seagateDeviceStats->sataStatistics.sanitizeCryptoErasePassCount.isSupported =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT63;
                    seagateDeviceStats->sataStatistics.sanitizeCryptoErasePassCount.isValueValid =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT62;
                    seagateDeviceStats->sataStatistics.sanitizeCryptoErasePassCount.isNormalized =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT61;
                ...
                case 1:
                    ...
                    seagateDeviceStats->sataStatistics.sanitizeCryptoErasePassTimeStamp.isSupported =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT63;
                    seagateDeviceStats->sataStatistics.sanitizeCryptoErasePassTimeStamp.isValueValid =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT62;
                    seagateDeviceStats->sataStatistics.sanitizeCryptoErasePassTimeStamp.isNormalized =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT61;
                ...
                case 2:
                    ...
                    seagateDeviceStats->sataStatistics.sanitizeOverwriteErasePassCount.isSupported =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT63;
                    seagateDeviceStats->sataStatistics.sanitizeOverwriteErasePassCount.isValueValid =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT62;
                    seagateDeviceStats->sataStatistics.sanitizeOverwriteErasePassCount.isNormalized =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT61;
                ...
                case 3:
                    ...
                    seagateDeviceStats->sataStatistics.sanitizeOverwriteErasePassTimeStamp.isSupported =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT63;
                    seagateDeviceStats->sataStatistics.sanitizeOverwriteErasePassTimeStamp.isValueValid =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT62;
                    seagateDeviceStats->sataStatistics.sanitizeOverwriteErasePassTimeStamp.isNormalized =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT61;
                ...
                case 4:
                    ...
                    seagateDeviceStats->sataStatistics.sanitizeBlockErasePassCount.isSupported =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT63;
                    seagateDeviceStats->sataStatistics.sanitizeBlockErasePassCount.isValueValid =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT62;
                    seagateDeviceStats->sataStatistics.sanitizeBlockErasePassCount.isNormalized =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT61;
                ...
                case 5:
                    ...
                    seagateDeviceStats->sataStatistics.sanitizeBlockErasePassTimeStamp.isSupported =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT63;
                    seagateDeviceStats->sataStatistics.sanitizeBlockErasePassTimeStamp.isValueValid =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT62;
                    seagateDeviceStats->sataStatistics.sanitizeBlockErasePassTimeStamp.isNormalized =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT61;
                ...
                case 6:
                    ...
                    seagateDeviceStats->sataStatistics.ataSecurityEraseUnitPassCount.isSupported =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT63;
                    seagateDeviceStats->sataStatistics.ataSecurityEraseUnitPassCount.isValueValid =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT62;
                    seagateDeviceStats->sataStatistics.ataSecurityEraseUnitPassCount.isNormalized =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT61;
                ...
                case 7:
                    ...
                    seagateDeviceStats->sataStatistics.ataSecurityEraseUnitPassTimeStamp.isSupported =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT63;
                    seagateDeviceStats->sataStatistics.ataSecurityEraseUnitPassTimeStamp.isValueValid =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT62;
                    seagateDeviceStats->sataStatistics.ataSecurityEraseUnitPassTimeStamp.isNormalized =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT61;
                ...
                case 8:
                    ...
                    seagateDeviceStats->sataStatistics.eraseSecurityFileFailureCount.isSupported =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT63;
                    seagateDeviceStats->sataStatistics.eraseSecurityFileFailureCount.isValueValid =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT62;
                    seagateDeviceStats->sataStatistics.eraseSecurityFileFailureCount.isNormalized =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT61;
                ...
                case 9:
                    ...
                    seagateDeviceStats->sataStatistics.eraseSecurityFileFailureTimeStamp.isSupported =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT63;
                    seagateDeviceStats->sataStatistics.eraseSecurityFileFailureTimeStamp.isValueValid =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT62;
                    seagateDeviceStats->sataStatistics.eraseSecurityFileFailureTimeStamp.isNormalized =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT61;
                ...
                case 10:
                    ...
                    seagateDeviceStats->sataStatistics.ataSecurityEraseUnitEnhancedPassCount.isSupported =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT63;
                    seagateDeviceStats->sataStatistics.ataSecurityEraseUnitEnhancedPassCount.isValueValid =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT62;
                    seagateDeviceStats->sataStatistics.ataSecurityEraseUnitEnhancedPassCount.isNormalized =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT61;
                ...
                case 11:
                    ...
                    seagateDeviceStats->sataStatistics.ataSecurityEraseUnitEnhancedPassTimeStamp.isSupported =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT63;
                    seagateDeviceStats->sataStatistics.ataSecurityEraseUnitEnhancedPassTimeStamp.isValueValid =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT62;
                    seagateDeviceStats->sataStatistics.ataSecurityEraseUnitEnhancedPassTimeStamp.isNormalized =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT61;
                ...
                case 12:
                    ...
                    seagateDeviceStats->sataStatistics.sanitizeCryptoEraseFailCount.isSupported =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT63;
                    seagateDeviceStats->sataStatistics.sanitizeCryptoEraseFailCount.isValueValid =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT62;
                    seagateDeviceStats->sataStatistics.sanitizeCryptoEraseFailCount.isNormalized =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT61;
                ...
                case 13:
                    ...
                    seagateDeviceStats->sataStatistics.sanitizeCryptoEraseFailTimeStamp.isSupported =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT63;
                    seagateDeviceStats->sataStatistics.sanitizeCryptoEraseFailTimeStamp.isValueValid =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT62;
                    seagateDeviceStats->sataStatistics.sanitizeCryptoEraseFailTimeStamp.isNormalized =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT61;
                ...
                case 14:
                    ...
                    seagateDeviceStats->sataStatistics.sanitizeOverwriteEraseFailCount.isSupported =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT63;
                    seagateDeviceStats->sataStatistics.sanitizeOverwriteEraseFailCount.isValueValid =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT62;
                    seagateDeviceStats->sataStatistics.sanitizeOverwriteEraseFailCount.isNormalized =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT61;
                ...
                case 15:
                    ...
                    seagateDeviceStats->sataStatistics.sanitizeOverwriteEraseFailTimeStamp.isSupported =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT63;
                    seagateDeviceStats->sataStatistics.sanitizeOverwriteEraseFailTimeStamp.isValueValid =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT62;
                    seagateDeviceStats->sataStatistics.sanitizeOverwriteEraseFailTimeStamp.isNormalized =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT61;
                ...
                case 16:
                    ...
                    seagateDeviceStats->sataStatistics.sanitizeBlockEraseFailCount.isSupported =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT63;
                    seagateDeviceStats->sataStatistics.sanitizeBlockEraseFailCount.isValueValid =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT62;
                    seagateDeviceStats->sataStatistics.sanitizeBlockEraseFailCount.isNormalized =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT61;
                ...
                case 17:
                    ...
                    seagateDeviceStats->sataStatistics.sanitizeBlockEraseFailTimeStamp.isSupported =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT63;
                    seagateDeviceStats->sataStatistics.sanitizeBlockEraseFailTimeStamp.isValueValid =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT62;
                    seagateDeviceStats->sataStatistics.sanitizeBlockEraseFailTimeStamp.isNormalized =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT61;
                ...
                case 18:
                    ...
                    seagateDeviceStats->sataStatistics.ataSecurityEraseUnitFailCount.isSupported =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT63;
                    seagateDeviceStats->sataStatistics.ataSecurityEraseUnitFailCount.isValueValid =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT62;
                    seagateDeviceStats->sataStatistics.ataSecurityEraseUnitFailCount.isNormalized =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT61;
                ...
                case 19:
                    ...
                    seagateDeviceStats->sataStatistics.ataSecurityEraseUnitFailTimeStamp.isSupported =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT63;
                    seagateDeviceStats->sataStatistics.ataSecurityEraseUnitFailTimeStamp.isValueValid =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT62;
                    seagateDeviceStats->sataStatistics.ataSecurityEraseUnitFailTimeStamp.isNormalized =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT61;
                ...
                case 20:
                    ...
                    seagateDeviceStats->sataStatistics.ataSecurityEraseUnitEnhancedFailCount.isSupported =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT63;
                    seagateDeviceStats->sataStatistics.ataSecurityEraseUnitEnhancedFailCount.isValueValid =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT62;
                    seagateDeviceStats->sataStatistics.ataSecurityEraseUnitEnhancedFailCount.isNormalized =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT61;
                ...
                case 21:
                    ...
                    seagateDeviceStats->sataStatistics.ataSecurityEraseUnitEnhancedFailTimeStamp.isSupported =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT63;
                    seagateDeviceStats->sataStatistics.ataSecurityEraseUnitEnhancedFailTimeStamp.isValueValid =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT62;
                    seagateDeviceStats->sataStatistics.ataSecurityEraseUnitEnhancedFailTimeStamp.isNormalized =
                        le64_to_host(qwordPtrDeviceStatsLog[0]) & BIT61;
...
typedef enum eSeagateSMARTStatusLogPageParamCodeEnum
{
                                                       ...
                                                       ptrSeagateDeviceStatistics seagateDeviceStats)
{
    ...
    if (SUCCESS == get_SCSI_Log_Size(device, 0x2F, 0x00, &deviceStatsSize))
    {
        ...
        if (SUCCESS ==
            get_SCSI_Log(device, 0x2F, 0x00, M_NULLPTR, M_NULLPTR, true, deviceStatsLog, deviceStatsSize, M_NULLPTR))
        {
            ret = SUCCESS;

            uint16_t pageLength      = M_BytesTo2ByteValue(deviceStatsLog[2], deviceStatsLog[3]);
            uint8_t  parameterLength = UINT8_C(0);
            for (uint16_t iter = UINT16_C(4); iter < pageLength;
                 iter += C_CAST(uint16_t, parameterLength + UINT16_C(4)))
            {
                ...
                case SANITIZE_CRYPTO_ERASE_STATISTICS:
                    ...
                    seagateDeviceStats->sasStatistics.sanitizeCryptoEraseCount.statisticsDataValue =
                        M_BytesTo4ByteValue(deviceStatsLog[iter + 4], deviceStatsLog[iter + 5],
                                            deviceStatsLog[iter + 6], deviceStatsLog[iter + 7]);
                    if (seagateDeviceStats->sasStatistics.sanitizeCryptoEraseCount.statisticsDataValue != 0)
                    {
                        ...
                        seagateDeviceStats->sasStatistics.sanitizeCryptoEraseTimeStamp.statisticsDataValue =
                            M_BytesTo4ByteValue(deviceStatsLog[iter + 8], deviceStatsLog[iter + 9],
                                                deviceStatsLog[iter + 10], deviceStatsLog[iter + 11]);
                    }
                    break;

                case SANITIZE_OVERWRITE_STATISTICS:
                    seagateDeviceStats->sasStatistics.sanitizeOverwriteEraseCount.isValueValid = true;
                    seagateDeviceStats->sasStatistics.sanitizeOverwriteEraseCount.statisticsDataValue =
                        M_BytesTo4ByteValue(deviceStatsLog[iter + 4], deviceStatsLog[iter + 5],
                                            deviceStatsLog[iter + 6], deviceStatsLog[iter + 7]);
                    if (seagateDeviceStats->sasStatistics.sanitizeOverwriteEraseCount.statisticsDataValue != 0)
                    {
                        ...
                        seagateDeviceStats->sasStatistics.sanitizeOverwriteEraseTimeStamp.statisticsDataValue =
                            M_BytesTo4ByteValue(deviceStatsLog[iter + 8], deviceStatsLog[iter + 9],
                                                deviceStatsLog[iter + 10], deviceStatsLog[iter + 11]);
                    }
                    break;
                ...
                case SANITIZE_BLOCK_ERASE_STATISTICS:
                    ...
                    seagateDeviceStats->sasStatistics.sanitizeBlockEraseCount.statisticsDataValue =
                        M_BytesTo4ByteValue(deviceStatsLog[iter + 4], deviceStatsLog[iter + 5],
                                            deviceStatsLog[iter + 6], deviceStatsLog[iter + 7]);
                    if (seagateDeviceStats->sasStatistics.sanitizeBlockEraseCount.statisticsDataValue != 0)
                    {
                        ...
                        seagateDeviceStats->sasStatistics.sanitizeBlockEraseTimeStamp.statisticsDataValue =
                            M_BytesTo4ByteValue(deviceStatsLog[iter + 8], deviceStatsLog[iter + 9],
                                                deviceStatsLog[iter + 10], deviceStatsLog[iter + 11]);
                    }
                    break;
                ...
                case ERASE_SECURITY_FILE_FAILURES:
                    ...
                    seagateDeviceStats->sasStatistics.eraseSecurityFileFailureCount.statisticsDataValue =
                        M_BytesTo4ByteValue(deviceStatsLog[iter + 4], deviceStatsLog[iter + 5],
                                            deviceStatsLog[iter + 6], deviceStatsLog[iter + 7]);
                    if (seagateDeviceStats->sasStatistics.eraseSecurityFileFailureCount.statisticsDataValue != 0)
                    {
                        ...
                        seagateDeviceStats->sasStatistics.eraseSecurityFileFailureTimeStamp.isTimeStampsInMinutes =
                            true;
                        seagateDeviceStats->sasStatistics.eraseSecurityFileFailureTimeStamp.statisticsDataValue =
                            M_BytesTo4ByteValue(deviceStatsLog[iter + 8], deviceStatsLog[iter + 9],
                                                deviceStatsLog[iter + 10], deviceStatsLog[iter + 11]);
                    }
                    break;
...
static void print_Seagate_ATA_DeviceStatistics(ptrSeagateDeviceStatistics seagateDeviceStats)
{
    if (seagateDeviceStats != M_NULLPTR)
    {
        ...
        for (uint8_t logEntry = UINT8_C(0); logEntry < maxLogEntries; ++logEntry)
        {
            ...
            {
            case 0:
                print_Count_Statistics("Sanitize Crypto Erase Count",
                                       seagateDeviceStats->sataStatistics.sanitizeCryptoErasePassCount);
                break;

            case 1:
                print_TimeStamp_Statistics("Sanitize Crypto Erase Timestamp",
                                           seagateDeviceStats->sataStatistics.sanitizeCryptoErasePassTimeStamp);
                break;

            case 2:
                print_Count_Statistics("Sanitize Overwrite Count",
                                       seagateDeviceStats->sataStatistics.sanitizeOverwriteErasePassCount);
                break;

            case 3:
                print_TimeStamp_Statistics("Sanitize Overwrite Timestamp",
                                           seagateDeviceStats->sataStatistics.sanitizeOverwriteErasePassTimeStamp);
                break;

            case 4:
                print_Count_Statistics("Sanitize Block Erase Count",
                                       seagateDeviceStats->sataStatistics.sanitizeBlockErasePassCount);
                break;

            case 5:
                print_TimeStamp_Statistics("Sanitize Block Erase Timestamp",
                                           seagateDeviceStats->sataStatistics.sanitizeBlockErasePassTimeStamp);
                break;

            case 6:
                print_Count_Statistics("ATA Security Erase Unit Count",
                                       seagateDeviceStats->sataStatistics.ataSecurityEraseUnitPassCount);
                break;

            case 7:
                print_TimeStamp_Statistics("ATA Security Erase Unit Timestamp",
                                           seagateDeviceStats->sataStatistics.ataSecurityEraseUnitPassTimeStamp);
                break;

            case 8:
                print_Count_Statistics("Erase Security File Failure Count",
                                       seagateDeviceStats->sataStatistics.eraseSecurityFileFailureCount);
                break;

            case 9:
                print_TimeStamp_Statistics("Erase Security File Failure Timestamp",
                                           seagateDeviceStats->sataStatistics.eraseSecurityFileFailureTimeStamp);
                break;

            case 10:
                print_Count_Statistics("ATA Security Erase Unit Enhanced Count",
                                       seagateDeviceStats->sataStatistics.ataSecurityEraseUnitEnhancedPassCount);
                break;

            case 11:
                print_TimeStamp_Statistics(
                    "ATA Security Erase Unit Enhanced Timestamp",
                    seagateDeviceStats->sataStatistics.ataSecurityEraseUnitEnhancedPassTimeStamp);
                break;

            case 12:
                print_Count_Statistics("Sanitize Crypto Erase Failure Count",
                                       seagateDeviceStats->sataStatistics.sanitizeCryptoEraseFailCount);
                break;

            case 13:
                print_TimeStamp_Statistics("Sanitize Crypto Erase Failure Timestamp",
                                           seagateDeviceStats->sataStatistics.sanitizeCryptoEraseFailTimeStamp);
                break;

            case 14:
                print_Count_Statistics("Sanitize Overwrite Failure Count",
                                       seagateDeviceStats->sataStatistics.sanitizeOverwriteEraseFailCount);
                break;

            case 15:
                print_TimeStamp_Statistics("Sanitize Overwrite Failure Timestamp",
                                           seagateDeviceStats->sataStatistics.sanitizeOverwriteEraseFailTimeStamp);
                break;

            case 16:
                print_Count_Statistics("Sanitize Block Erase Failure Count",
                                       seagateDeviceStats->sataStatistics.sanitizeBlockEraseFailCount);
                break;

            case 17:
                print_TimeStamp_Statistics("Sanitize Block Erase Failure Timestamp",
                                           seagateDeviceStats->sataStatistics.sanitizeBlockEraseFailTimeStamp);
                break;

            case 18:
                print_Count_Statistics("ATA Security Erase Unit Failure Count",
                                       seagateDeviceStats->sataStatistics.ataSecurityEraseUnitFailCount);
                break;

            case 19:
                print_TimeStamp_Statistics("ATA Security Erase Unit Failure Timestamp",
                                           seagateDeviceStats->sataStatistics.ataSecurityEraseUnitFailTimeStamp);
                break;

            case 20:
                print_Count_Statistics("ATA Security Erase Unit Enhanced Failure Count",
                                       seagateDeviceStats->sataStatistics.ataSecurityEraseUnitEnhancedFailCount);
                break;

            case 21:
                print_TimeStamp_Statistics(
                    "ATA Security Erase Unit Enhanced Failure Timestamp",
                    seagateDeviceStats->sataStatistics.ataSecurityEraseUnitEnhancedFailTimeStamp);
                break;
        ...
        if (seagateDeviceStats->sataStatistics.sanitizeCryptoErasePassCount.isValueValid &&
            seagateDeviceStats->sataStatistics.sanitizeCryptoEraseFailCount.isValueValid)
        {
            uint64_t timestampInMinutesForPass = UINT64_C(0);
            uint64_t timestampInMinutesForFail = UINT64_C(0);
            if (seagateDeviceStats->sataStatistics.sanitizeCryptoErasePassTimeStamp.isValueValid)
            {
                timestampInMinutesForPass =
                    seagateDeviceStats->sataStatistics.sanitizeCryptoErasePassTimeStamp.isTimeStampsInMinutes
                        ? seagateDeviceStats->sataStatistics.sanitizeCryptoErasePassTimeStamp.statisticsDataValue
                        : seagateDeviceStats->sataStatistics.sanitizeCryptoErasePassTimeStamp.statisticsDataValue *
                              UINT64_C(60);
...
static void print_Seagate_SCSI_DeviceStatistics(ptrSeagateDeviceStatistics seagateDeviceStats)
{
    if (seagateDeviceStats != M_NULLPTR)
    {
        ...
        print_Count_Statistics("Sanitize Crypo Erase Count",
                               seagateDeviceStats->sasStatistics.sanitizeCryptoEraseCount);
        print_TimeStamp_Statistics("Sanitize Crypo Erase Requested Time",
                                   seagateDeviceStats->sasStatistics.sanitizeCryptoEraseTimeStamp);

        print_Count_Statistics("Sanitize Overwrite Erase Count",
                               seagateDeviceStats->sasStatistics.sanitizeOverwriteEraseCount);
        print_TimeStamp_Statistics("Sanitize Overwrite Erase Requested Time",
                                   seagateDeviceStats->sasStatistics.sanitizeOverwriteEraseTimeStamp);

        print_Count_Statistics("Sanitize Block Erase Count", seagateDeviceStats->sasStatistics.sanitizeBlockEraseCount);
        print_TimeStamp_Statistics("Sanitize Block Erase Requested Time",
                                   seagateDeviceStats->sasStatistics.sanitizeBlockEraseTimeStamp);

        print_Count_Statistics("Erase Security File Failures Count",
                               seagateDeviceStats->sasStatistics.eraseSecurityFileFailureCount);
        print_TimeStamp_Statistics("Erase Security File Failures Requested Time",
                                   seagateDeviceStats->sasStatistics.eraseSecurityFileFailureTimeStamp);
    }
}
