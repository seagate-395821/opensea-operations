...
static bool add_Feature_To_Supported_List(char        featuresSupported[MAX_FEATURES][MAX_FEATURE_LENGTH],
                                          ...
                                          const char* featureString)
{
    ...
    if (featuresSupported && numberOfFeaturesSupported && featureString)
    {
        ...
        else
        {
            success = false;
#if defined(_DEBUG)
            print_str("Out of room in feature list!\n");
#endif //_DEBUG
        }
    }
    else
...
static bool add_Specification_To_Supported_List(char        specificationsSupported[MAX_SPECS][MAX_SPEC_LENGTH],
                                                uint8_t*    numberOfSpecificationsSupported,
                                                const char* specificationString)
{
    bool success = true;
    if (specificationsSupported && numberOfSpecificationsSupported && specificationString)
    {
        ...
        else
        {
            success = false;
#if defined(_DEBUG)
            print_str("Out of room in specification list!\n");
#endif //_DEBUG
        }
    }
    else
...
// This is an internal structure that's purpose is to report capabilities out of the subfunctions processing identify
// data or other data so that additional commands to run for device discovery can be performed as needed based on these
// capabilities.
typedef struct s_idDataCapabilitiesForDriveInfo
{
    eSeagateFamily seagateFamily;
    bool supportsIDDataLog; // used to help figure out what needs to be parsed or not out of std identify data, not
                            // already available in the log
    struct
    {
        bool copyOfIdentify;
        bool capacity;
                                                      ...
                                                      uint32_t                          dataLength)
{
    ...
    if (is_ATA_Identify_Word_Valid(le16_to_host(wordPtr[80])))
    {
        ...
        {
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "ACS-8");
        }
        if (specsBits & BIT14)
        {
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "ACS-7");
        }
        if (specsBits & BIT13)
        {
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "ACS-6");
        }
        if (specsBits & BIT12)
        {
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "ACS-5");
        }
        if (specsBits & BIT11)
        {
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "ACS-4");
        }
        if (specsBits & BIT10)
        {
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "ACS-3");
        }
        if (specsBits & BIT9)
        {
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "ACS-2");
        }
        if (specsBits & BIT8)
        {
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "ATA8-ACS");
        }
        if (specsBits & BIT7)
        {
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "ATA/ATAPI-7");
        }
        if (specsBits & BIT6)
        {
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "ATA/ATAPI-6");
        }
        if (specsBits & BIT5)
        {
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "ATA/ATAPI-5");
        }
        if (specsBits & BIT4)
        {
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "ATA/ATAPI-4");
        }
        if (specsBits & BIT3)
        {
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "ATA-3");
        }
        if (specsBits & BIT2)
        {
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "ATA-2");
        }
        if (specsBits & BIT1)
        {
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "ATA-1");
        }
    }
    else
    {
        // if this was not reported, assume ATA-1
        // NOTE: May be able to check other fields to determine if a later standard is supported or not, but this is a
        // fair assumption-TJE
        add_Specification_To_Supported_List(driveInfo->specificationsSupported,
    ...
    if (is_ATA_Identify_Word_Valid(le16_to_host(wordPtr[81])))
    {
        ...
        case ATA_MINOR_VERSION_ATA_1_PRIOR_TO_REV_4:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "ATA-1 (pre Revision 4)");
            break;
        case ATA_MINOR_VERSION_ATA_1_PUBLISHED:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "ATA-1 (Published)");
            break;
        case ATA_MINOR_VERSION_ATA_1_REV_4:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "ATA-1 (Revision 4)");
            break;
        case ATA_MINOR_VERSION_ATA_2_PUBLISHED:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "ATA-2 (Published)");
            break;
        case ATA_MINOR_VERSION_ATA_2_PRIOR_TO_REV_2K:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "ATA-2 (Pre Revision 2K)");
            break;
        case ATA_MINOR_VERSION_ATA_3_REV_1:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "ATA-3 (Revision 1)");
            break;
        case ATA_MINOR_VERSION_ATA_2_REV_2K:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "ATA-2 (Revision 2K)");
            break;
        case ATA_MINOR_VERSION_ATA_3_REV_0:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "ATA-3 (Revision 0)");
            break;
        case ATA_MINOR_VERSION_ATA_2_REV_3:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "ATA-2 (Revision 3)");
            break;
        case ATA_MINOR_VERSION_ATA_3_PUBLISHED:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "ATA-3 (Published)");
            break;
        case ATA_MINOR_VERSION_ATA_3_REV_6:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "ATA-3 (Revision 6)");
            break;
        case ATA_MINOR_VERSION_ATA_3_REV_7_AND_7A:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "ATA-3 (Revision 7 & 7A)");
            break;
        case ATA_MINOR_VERSION_ATA_ATAPI_4_REV_6:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported,
                                                "ATA/ATAPI-4 (Revision 6)");
            break;
        case ATA_MINOR_VERSION_ATA_ATAPI_4_REV_13:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported,
                                                "ATA/ATAPI-4 (Revision 13)");
            break;
        case ATA_MINOR_VERSION_ATA_ATAPI_4_REV7:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported,
                                                "ATA/ATAPI-4 (Revision 7)");
            break;
        case ATA_MINOR_VERSION_ATA_ATAPI_4_REV_18:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported,
                                                "ATA/ATAPI-4 (Revision 18)");
            break;
        case ATA_MINOR_VERSION_ATA_ATAPI_4_REV_15:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported,
                                                "ATA/ATAPI-4 (Revision 15)");
            break;
        case ATA_MINOR_VERSION_ATA_ATAPI_4_PUBLISHED:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "ATA/ATAPI-4 (Published)");
            break;
        case ATA_MINOR_VERSION_ATA_ATAPI_5_REV_3:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported,
                                                "ATA/ATAPI-5 (Revision 3)");
            break;
        case ATA_MINOR_VERSION_ATA_ATAPI_4_REV_14:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported,
                                                "ATA/ATAPI-4 (Revision 14)");
            break;
        case ATA_MINOR_VERSION_ATA_ATAPI_5_REV_1:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported,
                                                "ATA/ATAPI-5 (Revision 1)");
            break;
        case ATA_MINOR_VERSION_ATA_ATAPI_5_PUBLISHED:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "ATA/ATAPI-5 (Published)");
            break;
        case ATA_MINOR_VERSION_ATA_ATAPI_4_REV_17:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported,
                                                "ATA/ATAPI-4 (Revision 17)");
            break;
        case ATA_MINOR_VERSION_ATA_ATAPI_6_REV_0:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported,
                                                "ATA/ATAPI-6 (Revision 0)");
            break;
        case ATA_MINOR_VERSION_ATA_ATAPI_6_REV_3A:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported,
                                                "ATA/ATAPI-6 (Revision 3A)");
            break;
        case ATA_MINOR_VERSION_ATA_ATAPI_7_REV_1:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported,
                                                "ATA/ATAPI-7 (Revision 1)");
            break;
        ...
        case ATA_MINOR_VERSION_ATA_ATAPI_6_REV_1:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported,
                                                "ATA/ATAPI-6 (Revision 1)");
            break;
        case ATA_MINOR_VERSION_ATA_ATAPI_7_RUBLISHED:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "ATA/ATAPI-7 (Published)");
            break;
        case ATA_MINOR_VERSION_ATA_ATAPI_7_REV_0:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported,
                                                "ATA/ATAPI-7 (Revision 0)");
            break;
        case ATA_MINOR_VERSION_ACS3_REV_3B:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "ACS-3 (Revision 3B)");
            break;
        case ATA_MINOR_VERSION_ATA_ATAPI_7_REV_4A:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported,
                                                "ATA/ATAPI-7 (Revision 4A)");
            break;
        case ATA_MINOR_VERSION_ATA_ATAPI_6_PUBLISHED:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "ATA/ATAPI-6 (Published)");
            break;
        case ATA_MINOR_VERSION_ATA8_ACS_REV_3C:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "ATA8-ACS (Revision 3C)");
            break;
        case ATA_MINOR_VERSION_ATA8_ACS_REV_6:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "ATA8-ACS (Revision 6)");
            break;
        case ATA_MINOR_VERSION_ATA8_ACS_REV_4:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "ATA8-ACS (Revision 4)");
            break;
        case ATA_MINOR_VERSION_ACS5_REV_8:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "ATA8-ACS (Revision 8)");
            break;
        case ATA_MINOR_VERSION_ACS2_REV_2:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "ACS-2 (Revision 2)");
            break;
        case ATA_MINOR_VERSION_ATA8_ACS_REV_3E:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "ATA8-ACS (Revision 3E)");
            break;
        case ATA_MINOR_VERSION_ATA8_ACS_REV_4C:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "ATA8-ACS (Revision 4C)");
            break;
        case ATA_MINOR_VERSION_ATA8_ACS_REV_3F:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "ATA8-ACS (Revision 3F)");
            break;
        case ATA_MINOR_VERSION_ATA8_ACS_REV_3B:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "ATA8-ACS (Revision 3B)");
            break;
        case ATA_MINOR_VERSION_ACS4_REV_5:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "ACS-4 (Revision 5)");
            break;
        case ATA_MINOR_VERSION_ACS3_REV_5:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "ACS-3 (Revision 5)");
            break;
        case ATA_MINOR_VERSION_ACS6_REV_2:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "ACS-6 (Revision 2)");
            break;
        case ATA_MINOR_VERSION_ACS_2_PUBLISHED:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "ACS-2 (Published)");
            break;
        case ATA_MINOR_VERSION_ACS4_PUBLISHED:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "ACS-4 (Published)");
            break;
        case ATA_MINOR_VERSION_ATA8_ACS_REV_2D:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "ATA8-ACS (Revision 2D)");
            break;
        case ATA_MINOR_VERSION_ACS3_PUBLISHED:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "ACS-3 (Published)");
            break;
        case ATA_MINOR_VERSION_ACS2_REV_3:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "ACS-2 (Revision 3)");
            break;
        case ATA_MINOR_VERSION_ACS3_REV_4:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "ACS-3 (Revision 4)");
            break;
    ...
    if (driveInfo->rotationRate == 0 && safe_strlen(driveInfo->modelNumber) > 0 &&
        (strstr(driveInfo->modelNumber, "Seagate SSD") != M_NULLPTR) && safe_strlen(driveInfo->firmwareRevision) > 0 &&
        (strstr(driveInfo->firmwareRevision, "UHFS") != M_NULLPTR))
    {
    ...
    if (is_ATA_Identify_Word_Valid(le16_to_host(wordPtr[222])))
    {
        ...
        if (specsBits & BIT10)
        {
            ...
            {
                add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                    &driveInfo->numberOfSpecificationsSupported, "SATA 3.5");
            }
            else
            {
                add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                    &driveInfo->numberOfSpecificationsSupported, "Reserved");
            }
        }
        if (specsBits & BIT9)
        {
            ...
            {
                add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                    &driveInfo->numberOfSpecificationsSupported, "SATA 3.4");
            }
            else
            {
                add_Specification_To_Supported_List(driveInfo->specificationsSupported,
        ...
        if (specsBits & BIT8)
        {
            ...
            {
                add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                    &driveInfo->numberOfSpecificationsSupported, "SATA 3.3");
            }
            else
            {
                add_Specification_To_Supported_List(driveInfo->specificationsSupported,
        ...
        if (specsBits & BIT7)
        {
            ...
            {
                add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                    &driveInfo->numberOfSpecificationsSupported, "SATA 3.2");
            }
            else
            {
                add_Specification_To_Supported_List(driveInfo->specificationsSupported,
        ...
        if (specsBits & BIT6)
        {
            ...
            {
                add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                    &driveInfo->numberOfSpecificationsSupported, "SATA 3.1");
            }
            else
            {
                add_Specification_To_Supported_List(driveInfo->specificationsSupported,
        ...
        if (specsBits & BIT5)
        {
            ...
            {
                add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                    &driveInfo->numberOfSpecificationsSupported, "SATA 3.0");
            }
            else
            {
                add_Specification_To_Supported_List(driveInfo->specificationsSupported,
        ...
        if (specsBits & BIT4)
        {
            ...
            {
                add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                    &driveInfo->numberOfSpecificationsSupported, "SATA 2.6");
            }
            else
            {
                add_Specification_To_Supported_List(driveInfo->specificationsSupported,
        ...
        if (specsBits & BIT3)
        {
            ...
            {
                add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                    &driveInfo->numberOfSpecificationsSupported, "SATA 2.5");
            }
            else
            {
                add_Specification_To_Supported_List(driveInfo->specificationsSupported,
        ...
        if (specsBits & BIT2)
        {
            ...
            {
                add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                    &driveInfo->numberOfSpecificationsSupported, "SATA II: Extensions");
            }
            else
            {
                add_Specification_To_Supported_List(driveInfo->specificationsSupported,
        ...
        if (specsBits & BIT1)
        {
            ...
            {
                add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                    &driveInfo->numberOfSpecificationsSupported, "SATA 1.0a");
            }
        ...
        if (specsBits & BIT0)
        {
            ...
            {
                add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                    &driveInfo->numberOfSpecificationsSupported, "ATA8-AST");
            }
            else if (transportType == 0)
            {
                add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                    &driveInfo->numberOfSpecificationsSupported, "ATA8-APT");
            }
            else
            {
                add_Specification_To_Supported_List(driveInfo->specificationsSupported,
    ...
    if (is_ATA_Identify_Word_Valid(le16_to_host(wordPtr[223])))
    {
        ...
        case TRANSPORT_MINOR_VERSION_ATA8_AST_D1697_VERSION_1:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported,
                                                "ATA8-AST T13 Project D1697 Version 1");
            break;
...
static eReturnValues get_ATA_Drive_Info_From_ID_Data_Log(ptrDriveInformationSAS_SATA       driveInfo,
                                                         ptrIdDataCapabilitiesForDriveInfo ataCapabilities,
                                                         uint8_t*                          idDataLog,
                                                         uint32_t                          dataLength)
{
    ...
    if (pageNumber == C_CAST(uint8_t, ATA_ID_DATA_LOG_SUPPORTED_PAGES) && revision >= ATA_ID_DATA_VERSION_1)
    {
        ...
        {
            uint64_t qword0 = M_BytesTo8ByteValue(idDataLog[offset + 7], idDataLog[offset + 6], idDataLog[offset + 5],
                                                  idDataLog[offset + 4], idDataLog[offset + 3], idDataLog[offset + 2],
                                                  idDataLog[offset + 1], idDataLog[offset + 0]);
            if (qword0 & ATA_ID_DATA_QWORD_VALID_BIT && M_Byte2(qword0) == ATA_ID_DATA_LOG_CAPACITY &&
                M_Word0(qword0) >= ATA_ID_DATA_VERSION_1)
            {
                // get the nominal buffer size
                uint64_t nominalBufferSize = M_BytesTo8ByteValue(
                    idDataLog[offset + 39], idDataLog[offset + 38], idDataLog[offset + 37], idDataLog[offset + 36],
                    idDataLog[offset + 35], idDataLog[offset + 34], idDataLog[offset + 33], idDataLog[offset + 32]);
        ...
        if (ataCapabilities->supportedIDDataPages.supportedCapabilities &&
            (offset + ATA_LOG_PAGE_LEN_BYTES) <= dataLength)
        {
            // supported capabilities
            uint64_t qword0 = M_BytesTo8ByteValue(idDataLog[offset + 7], idDataLog[offset + 6], idDataLog[offset + 5],
                                                  idDataLog[offset + 4], idDataLog[offset + 3], idDataLog[offset + 2],
                                                  idDataLog[offset + 1], idDataLog[offset + 0]);
            if (qword0 & ATA_ID_DATA_QWORD_VALID_BIT && M_Byte2(qword0) == ATA_ID_DATA_LOG_SUPPORTED_CAPABILITIES &&
                M_Word0(qword0) >= ATA_ID_DATA_VERSION_1)
            {
                uint64_t supportedCapabilitiesQWord = M_BytesTo8ByteValue(
                    idDataLog[offset + 15], idDataLog[offset + 14], idDataLog[offset + 13], idDataLog[offset + 12],
                    idDataLog[offset + 11], idDataLog[offset + 10], idDataLog[offset + 9], idDataLog[offset + 8]);
                if (supportedCapabilitiesQWord & ATA_ID_DATA_QWORD_VALID_BIT)
                {
                    if (supportedCapabilitiesQWord & BIT55)
                    {
                        add_Feature_To_Supported_List(driveInfo->featuresSupported,
                                                      &driveInfo->numberOfFeaturesSupported, "Mutate");
                    }
                ...
                uint64_t downloadCapabilities = M_BytesTo8ByteValue(
                    idDataLog[offset + 23], idDataLog[offset + 22], idDataLog[offset + 21], idDataLog[offset + 20],
                    idDataLog[offset + 19], idDataLog[offset + 18], idDataLog[offset + 17], idDataLog[offset + 16]);
                ...
                // zoned capabilities
                uint64_t zonedCapabilities = M_BytesTo8ByteValue(
                    idDataLog[offset + 111], idDataLog[offset + 110], idDataLog[offset + 109], idDataLog[offset + 108],
                    idDataLog[offset + 107], idDataLog[offset + 106], idDataLog[offset + 105], idDataLog[offset + 104]);
                ...
                uint64_t supportedCapabilitiesQWord18 = M_BytesTo8ByteValue(
                    idDataLog[offset + 159], idDataLog[offset + 158], idDataLog[offset + 157], idDataLog[offset + 156],
                    idDataLog[offset + 155], idDataLog[offset + 154], idDataLog[offset + 153], idDataLog[offset + 152]);
                if (supportedCapabilitiesQWord18 & ATA_ID_DATA_QWORD_VALID_BIT) // making sure this is set for
                                                                                // "validity"
                {
                    if (supportedCapabilitiesQWord18 & BIT1 &&
                        supportedCapabilitiesQWord18 & BIT0) // checking for both commands to be supported
                    {
                ...
                uint64_t cdlSupportQword = M_BytesTo8ByteValue(
                    idDataLog[offset + 175], idDataLog[offset + 174], idDataLog[offset + 173], idDataLog[offset + 172],
                    idDataLog[offset + 171], idDataLog[offset + 170], idDataLog[offset + 169], idDataLog[offset + 168]);
        ...
        if (ataCapabilities->supportedIDDataPages.currentSettings && (offset + ATA_LOG_PAGE_LEN_BYTES) <= dataLength)
        {
            uint64_t qword0 = M_BytesTo8ByteValue(idDataLog[offset + 7], idDataLog[offset + 6], idDataLog[offset + 5],
                                                  idDataLog[offset + 4], idDataLog[offset + 3], idDataLog[offset + 2],
                                                  idDataLog[offset + 1], idDataLog[offset + 0]);
            if (qword0 & ATA_ID_DATA_QWORD_VALID_BIT && M_Byte2(qword0) == ATA_ID_DATA_LOG_CURRENT_SETTINGS &&
                M_Word0(qword0) >= ATA_ID_DATA_VERSION_1)
            {
        ...
        if (cdlSupported)
        {
            ...
            {
                add_Feature_To_Supported_List(driveInfo->featuresSupported, &driveInfo->numberOfFeaturesSupported,
                                              "Command Duration Limits [Enabled]");
            }
            else
            {
                add_Feature_To_Supported_List(driveInfo->featuresSupported, &driveInfo->numberOfFeaturesSupported,
        ...
        {
            uint64_t qword0 = M_BytesTo8ByteValue(idDataLog[offset + 7], idDataLog[offset + 6], idDataLog[offset + 5],
        idDataLog[offset + 4], idDataLog[offset + 3], idDataLog[offset + 2], idDataLog[offset + 1], idDataLog[offset +
        0]); if (qword0 & ATA_ID_DATA_QWORD_VALID_BIT && M_Byte2(qword0) == ATA_ID_DATA_LOG_ATA_STRINGS &&
        M_Word0(qword0) >= ATA_ID_DATA_VERSION_1)
            {
            }
        }
        ...
        if (ataCapabilities->supportedIDDataPages.security && (offset + ATA_LOG_PAGE_LEN_BYTES) <= dataLength)
        {
            uint64_t qword0 = M_BytesTo8ByteValue(idDataLog[offset + 7], idDataLog[offset + 6], idDataLog[offset + 5],
        idDataLog[offset + 4], idDataLog[offset + 3], idDataLog[offset + 2], idDataLog[offset + 1], idDataLog[offset +
        0]); if (qword0 & ATA_ID_DATA_QWORD_VALID_BIT && M_Byte2(qword0) == ATA_ID_DATA_LOG_SECURITY && M_Word0(qword0)
        >= ATA_ID_DATA_VERSION_1)
            {
            }
        }
        ...
        if (ataCapabilities->supportedIDDataPages.parallelATA && (offset + ATA_LOG_PAGE_LEN_BYTES) <= dataLength)
        {
            uint64_t qword0 = M_BytesTo8ByteValue(idDataLog[offset + 7], idDataLog[offset + 6], idDataLog[offset + 5],
        idDataLog[offset + 4], idDataLog[offset + 3], idDataLog[offset + 2], idDataLog[offset + 1], idDataLog[offset +
        0]); if (qword0 & ATA_ID_DATA_QWORD_VALID_BIT && M_Byte2(qword0) == ATA_ID_DATA_LOG_PARALLEL_ATA &&
        M_Word0(qword0) >= ATA_ID_DATA_VERSION_1)
            {
            }
        }
        ...
        if (ataCapabilities->supportedIDDataPages.serialATA && (offset + ATA_LOG_PAGE_LEN_BYTES) <= dataLength)
        {
            uint64_t qword0 = M_BytesTo8ByteValue(idDataLog[offset + 7], idDataLog[offset + 6], idDataLog[offset + 5],
        idDataLog[offset + 4], idDataLog[offset + 3], idDataLog[offset + 2], idDataLog[offset + 1], idDataLog[offset +
        0]); if (qword0 & ATA_ID_DATA_QWORD_VALID_BIT && M_Byte2(qword0) == ATA_ID_DATA_LOG_SERIAL_ATA &&
        M_Word0(qword0) >= ATA_ID_DATA_VERSION_1)
            {
            }
        }*/
        ...
        if (ataCapabilities->supportedIDDataPages.zac2 && (offset + ATA_LOG_PAGE_LEN_BYTES) <= dataLength)
        {
            uint64_t qword0 = M_BytesTo8ByteValue(idDataLog[offset + 7], idDataLog[offset + 6], idDataLog[offset + 5],
                                                  idDataLog[offset + 4], idDataLog[offset + 3], idDataLog[offset + 2],
                                                  idDataLog[offset + 1], idDataLog[offset + 0]);
            if (qword0 & ATA_ID_DATA_QWORD_VALID_BIT && M_Byte2(qword0) == ATA_ID_DATA_LOG_ZONED_DEVICE_INFORMATION &&
                M_Word0(qword0) >= ATA_ID_DATA_VERSION_1)
            {
                uint64_t zonedSettingsQWord = M_BytesTo8ByteValue(
                    idDataLog[offset + 15], idDataLog[offset + 14], idDataLog[offset + 13], idDataLog[offset + 12],
                    idDataLog[offset + 11], idDataLog[offset + 10], idDataLog[offset + 9], idDataLog[offset + 8]);
                uint64_t versionQWord = M_BytesTo8ByteValue(
                    idDataLog[offset + 55], idDataLog[offset + 54], idDataLog[offset + 53], idDataLog[offset + 52],
                    idDataLog[offset + 51], idDataLog[offset + 50], idDataLog[offset + 49], idDataLog[offset + 48]);
                uint64_t zoneActCapQWord = M_BytesTo8ByteValue(
                    idDataLog[offset + 63], idDataLog[offset + 62], idDataLog[offset + 61], idDataLog[offset + 60],
                    idDataLog[offset + 59], idDataLog[offset + 58], idDataLog[offset + 57], idDataLog[offset + 56]);
        ...
        if (ataCapabilities->supportedIDDataPages.copyOfIdentify && !ataCapabilities->processedStdIDData)
        {
            ...
            // std identify data was not already processed, so process it here for anything not covered by the above
            // code.
...
// this function expects a complete ID data log, so the supported pages followed by all supported pages that were read.
static eReturnValues get_ATA_Drive_Info_From_Device_Statistics_Log(ptrDriveInformationSAS_SATA       driveInfo,
                                                                   ptrIdDataCapabilitiesForDriveInfo ataCapabilities,
                                                                   uint8_t*                          idDataLog,
                                                                   uint32_t                          dataLength)
{
    ...
    if (pageNumber == C_CAST(uint8_t, ATA_DEVICE_STATS_LOG_LIST) && revision >= ATA_DEV_STATS_VERSION_1)
    {
        ...
        for (iter = ATA_DEV_STATS_SUP_PG_LIST_OFFSET;
             iter < (numberOfEntries + ATA_DEV_STATS_SUP_PG_LIST_OFFSET) && iter < ATA_LOG_PAGE_LEN_BYTES; ++iter)
        {
            switch (idDataLog[iter])
            {
            ...
            case ATA_DEVICE_STATS_LOG_GENERAL:
                ...
                break;
            case ATA_DEVICE_STATS_LOG_FREE_FALL: // NOLINT
                // freeFallStatistics = true;
                break;
            case ATA_DEVICE_STATS_LOG_ROTATING_MEDIA: // NOLINT
                // rotatingMediaStatistics = true;
                break;
            case ATA_DEVICE_STATS_LOG_GEN_ERR: // NOLINT
                // generalErrorStatistics = true;
                break;
            case ATA_DEVICE_STATS_LOG_TEMP:
                temperatureStatistics = true;
                break;
            case ATA_DEVICE_STATS_LOG_TRANSPORT: // NOLINT
                // transportStatistics = true;
                break;
            case ATA_DEVICE_STATS_LOG_SSD:
                solidStateStatistics = true;
                break;
            case ATA_DEVICE_STATS_LOG_ZONED_DEVICE: // NOLINT
        ...
        if (generalStatistics && (offset + ATA_LOG_PAGE_LEN_BYTES) <= dataLength)
        {
            ...
            {
                // power on hours
                uint64_t pohQword = M_BytesTo8ByteValue(
                    idDataLog[offset + 23], idDataLog[offset + 22], idDataLog[offset + 21], idDataLog[offset + 20],
                    idDataLog[offset + 19], idDataLog[offset + 18], idDataLog[offset + 17], idDataLog[offset + 16]);
                ...
                uint64_t lsWrittenQword = M_BytesTo8ByteValue(
                    idDataLog[offset + 31], idDataLog[offset + 30], idDataLog[offset + 29], idDataLog[offset + 28],
                    idDataLog[offset + 27], idDataLog[offset + 26], idDataLog[offset + 25], idDataLog[offset + 24]);
                ...
                uint64_t lsReadQword = M_BytesTo8ByteValue(
                    idDataLog[offset + 47], idDataLog[offset + 46], idDataLog[offset + 45], idDataLog[offset + 44],
                    idDataLog[offset + 43], idDataLog[offset + 42], idDataLog[offset + 41], idDataLog[offset + 40]);
                ...
                uint64_t worloadUtilization = M_BytesTo8ByteValue(
                    idDataLog[offset + 79], idDataLog[offset + 78], idDataLog[offset + 77], idDataLog[offset + 76],
                    idDataLog[offset + 75], idDataLog[offset + 74], idDataLog[offset + 73], idDataLog[offset + 72]);
        ...
        //{
        //     uint64_t qword0 = M_BytesTo8ByteValue(idDataLog[offset + 7], idDataLog[offset + 6], idDataLog[offset +
        //     5], idDataLog[offset + 4], idDataLog[offset + 3], idDataLog[offset + 2], idDataLog[offset + 1],
        //     idDataLog[offset + 0]); if (M_Byte2(qword0) == ATA_DEVICE_STATS_LOG_FREE_FALL && M_Word0(qword0) >=
        //     ATA_DEV_STATS_VERSION_1)//validating we got the right page
        //     {
        //     }
        // }
        ...
        //     uint64_t qword0 = M_BytesTo8ByteValue(idDataLog[offset + 7], idDataLog[offset + 6], idDataLog[offset +
        //     5], idDataLog[offset + 4], idDataLog[offset + 3], idDataLog[offset + 2], idDataLog[offset + 1],
        //     idDataLog[offset + 0]); if (M_Byte2(qword0) == ATA_DEVICE_STATS_LOG_ROTATING_MEDIA && M_Word0(qword0) >=
        //     ATA_DEV_STATS_VERSION_1)//validating we got the right page
        //     {
        //     }
        // }
        ...
        //     uint64_t qword0 = M_BytesTo8ByteValue(idDataLog[offset + 7], idDataLog[offset + 6], idDataLog[offset +
        //     5], idDataLog[offset + 4], idDataLog[offset + 3], idDataLog[offset + 2], idDataLog[offset + 1],
        //     idDataLog[offset + 0]); if (M_Byte2(qword0) == ATA_DEVICE_STATS_LOG_GEN_ERR && M_Word0(qword0) >=
        //     ATA_DEV_STATS_VERSION_1)//validating we got the right page
        //     {
        //     }
        // }
        ...
        if (temperatureStatistics && (offset + ATA_LOG_PAGE_LEN_BYTES) <= dataLength)
        {
            uint64_t qword0 = M_BytesTo8ByteValue(idDataLog[offset + 7], idDataLog[offset + 6], idDataLog[offset + 5],
                                                  idDataLog[offset + 4], idDataLog[offset + 3], idDataLog[offset + 2],
                                                  idDataLog[offset + 1], idDataLog[offset + 0]);
            if (M_Byte2(qword0) == ATA_DEVICE_STATS_LOG_TEMP &&
                M_Word0(qword0) >= ATA_DEV_STATS_VERSION_1) // validating we got the right page
            {
                // current temperature
                uint64_t currentTemp = M_BytesTo8ByteValue(
                    idDataLog[offset + 15], idDataLog[offset + 14], idDataLog[offset + 13], idDataLog[offset + 12],
                    idDataLog[offset + 11], idDataLog[offset + 10], idDataLog[offset + 9], idDataLog[offset + 8]);
                ...
                // highest temperature
                uint64_t highestTemp = M_BytesTo8ByteValue(
                    idDataLog[offset + 39], idDataLog[offset + 38], idDataLog[offset + 37], idDataLog[offset + 36],
                    idDataLog[offset + 35], idDataLog[offset + 34], idDataLog[offset + 33], idDataLog[offset + 32]);
                ...
                // lowest temperature
                uint64_t lowestTemp = M_BytesTo8ByteValue(
                    idDataLog[offset + 47], idDataLog[offset + 46], idDataLog[offset + 45], idDataLog[offset + 44],
                    idDataLog[offset + 43], idDataLog[offset + 42], idDataLog[offset + 41], idDataLog[offset + 40]);
        ...
        //     uint64_t qword0 = M_BytesTo8ByteValue(idDataLog[offset + 7], idDataLog[offset + 6], idDataLog[offset +
        //     5], idDataLog[offset + 4], idDataLog[offset + 3], idDataLog[offset + 2], idDataLog[offset + 1],
        //     idDataLog[offset + 0]); if (M_Byte2(qword0) == ATA_DEVICE_STATS_LOG_TRANSPORT && M_Word0(qword0) >=
        //     ATA_DEV_STATS_VERSION_1)//validating we got the right page
        //     {
        //     }
        // }
        ...
        if (solidStateStatistics && (offset + ATA_LOG_PAGE_LEN_BYTES) <= dataLength)
        {
            uint64_t qword0 = M_BytesTo8ByteValue(idDataLog[offset + 7], idDataLog[offset + 6], idDataLog[offset + 5],
                                                  idDataLog[offset + 4], idDataLog[offset + 3], idDataLog[offset + 2],
                                                  idDataLog[offset + 1], idDataLog[offset + 0]);
            if (M_Byte2(qword0) == ATA_DEVICE_STATS_LOG_SSD &&
                M_Word0(qword0) >= ATA_DEV_STATS_VERSION_1) // validating we got the right page
            {
                // percent used endurance
                uint64_t percentUsed = M_BytesTo8ByteValue(
                    idDataLog[offset + 15], idDataLog[offset + 14], idDataLog[offset + 13], idDataLog[offset + 12],
                    idDataLog[offset + 11], idDataLog[offset + 10], idDataLog[offset + 9], idDataLog[offset + 8]);
        ...
        //     uint64_t qword0 = M_BytesTo8ByteValue(idDataLog[offset + 7], idDataLog[offset + 6], idDataLog[offset +
        //     5], idDataLog[offset + 4], idDataLog[offset + 3], idDataLog[offset + 2], idDataLog[offset + 1],
        //     idDataLog[offset + 0]); if (M_Byte2(qword0) == ATA_DEVICE_STATS_LOG_ZONED_DEVICE && M_Word0(qword0) >=
        //     ATA_DEV_STATS_VERSION_1)//validating we got the right page
        //     {
        //     }
        // }
...
static eReturnValues get_ATA_Drive_Info_From_SMART_Data(ptrDriveInformationSAS_SATA       driveInfo,
                                                        ptrIdDataCapabilitiesForDriveInfo ataCapabilities,
                                                        uint8_t*                          smartData,
                                                        uint32_t                          dataLength)
{
    ...
    if (smartData && dataLength >= LEGACY_DRIVE_SEC_SIZE)
    {
        ...
        for (smartIter = UINT16_C(2); smartIter < UINT16_C(362); smartIter += UINT16_C(12))
        {
            ...
            {
                uint32_t millisecondsSinceIncrement = M_BytesTo4ByteValue(
                    0, currentAttribute.rawData[6], currentAttribute.rawData[5], currentAttribute.rawData[4]);
                uint32_t powerOnMinutes =
                    M_BytesTo4ByteValue(currentAttribute.rawData[3], currentAttribute.rawData[2],
                                        currentAttribute.rawData[1], currentAttribute.rawData[0]) *
                    60;
                powerOnMinutes +=
                    (millisecondsSinceIncrement /
                     60000); // convert the milliseconds to minutes, then add that to the amount of time we already know
            ...
            case 194:
                ...
                if (!driveInfo->temperatureData.temperatureDataValid)
                {
                    ...
                    driveInfo->temperatureData.currentTemperature =
                        C_CAST(int16_t, M_BytesTo2ByteValue(currentAttribute.rawData[1], currentAttribute.rawData[0]));
                }
                /* TODO: This can be improved with better filters/interpretations defined per vendor to read this
                 * attribute. */
                if (ataCapabilities->seagateFamily != MAXTOR)
                {
                    if (ataCapabilities->seagateFamily == SEAGATE_VENDOR_K)
                    {
                        if (!driveInfo->temperatureData.highestValid &&
                            currentAttribute.worstEver !=
                                ATA_SMART_ATTRIBUTE_WORST_COMMON_START) // Filter out 253 as that is an unreasonable
                                                                        // measurement, and more likely corresponds to
                                                                        // an unreported or unsupported value
                        {
                            driveInfo->temperatureData.highestTemperature =
                                C_CAST(int16_t, currentAttribute.worstEver); // or raw 5:4
                            driveInfo->temperatureData.highestValid = true;
                        }
                    }
                    else if (ataCapabilities->seagateFamily == SEAGATE_VENDOR_D ||
                             ataCapabilities->seagateFamily == SEAGATE_VENDOR_E)
                    {
                        if (!driveInfo->temperatureData.highestValid &&
                            currentAttribute.worstEver !=
                                ATA_SMART_ATTRIBUTE_WORST_COMMON_START) // Filter out 253 as that is an unreasonable
                                                                        ...
                                                                        // an unreported or unsupported value
                        {
                            driveInfo->temperatureData.highestTemperature = C_CAST(
                                int16_t, M_BytesTo2ByteValue(currentAttribute.rawData[3], currentAttribute.rawData[2]));
                            driveInfo->temperatureData.highestValid = true;
                        }
                    }
                    else
                    {
                        if (!driveInfo->temperatureData.lowestValid &&
                            C_CAST(int16_t,
                                   M_BytesTo2ByteValue(currentAttribute.rawData[5], currentAttribute.rawData[4])) <=
                                driveInfo->temperatureData.currentTemperature)
                        {
                            driveInfo->temperatureData.lowestTemperature = C_CAST(
                                int16_t, M_BytesTo2ByteValue(currentAttribute.rawData[5], currentAttribute.rawData[4]));
                            driveInfo->temperatureData.lowestValid = true;
                        }
                        if (!driveInfo->temperatureData.highestValid &&
                            currentAttribute.worstEver !=
            ...
            case 231: // SSD Endurance
                if ((ataCapabilities->seagateFamily == SEAGATE || ataCapabilities->seagateFamily == SEAGATE_VENDOR_D ||
                     ataCapabilities->seagateFamily == SEAGATE_VENDOR_E ||
                     ataCapabilities->seagateFamily == SEAGATE_VENDOR_C ||
                     ataCapabilities->seagateFamily == SEAGATE_VENDOR_F ||
                     ataCapabilities->seagateFamily == SEAGATE_VENDOR_G ||
                     ataCapabilities->seagateFamily == SEAGATE_VENDOR_K) &&
                    driveInfo->percentEnduranceUsed < 0)
                {
                    // SCSI was implemented first, and it returns a value where 0 means 100% spares left, ATA is the
                    // opposite, so we need to subtract our number from 100 On SATA drives below, we had firmware
                    // reporting in the range of 0-255 instead of 0-100. Lets check for them and normalize the value so
                    // it can be reported Note that this is only for some firmwares...we should figure out a better way
                    // to do this Here's the list of the affected SATA model numbers: ST100FM0022 (SED - rare)
                    ...
                    {
                        // We want integer division here to match what SHOULD be reported since this was a drive bug
                        uint8_t adjustedEndurance =
                            UINT8_C(100) - ((C_CAST(uint32_t, currentAttribute.nominal) * UINT8_C(100)) / UINT8_C(255));
                        driveInfo->percentEnduranceUsed = adjustedEndurance;
                    }
                    else
                    {
                        driveInfo->percentEnduranceUsed = UINT8_C(100) - currentAttribute.nominal;
                    }
                }
                break;
            case 233: // Lifetime Write to Flash (SSD)
                if (ataCapabilities->seagateFamily == SEAGATE_VENDOR_G ||
                    ataCapabilities->seagateFamily == SEAGATE_VENDOR_F)
                {
                    driveInfo->totalWritesToFlash = M_BytesTo8ByteValue(
                        0, currentAttribute.rawData[6], currentAttribute.rawData[5], currentAttribute.rawData[4],
                        currentAttribute.rawData[3], currentAttribute.rawData[2], currentAttribute.rawData[1],
                        currentAttribute.rawData[0]);
                    // convert this to match what we're doing below since this is likely also in GiB written (BUT IDK
                    // BECAUSE IT ISN'T IN THE SMART SPEC!)
                    driveInfo->totalWritesToFlash =
                        (driveInfo->totalWritesToFlash * 1024 * 1024 * 1024) / driveInfo->logicalSectorSize;
                }
                else if (ataCapabilities->seagateFamily == SEAGATE_VENDOR_K)
                {
                    ...
                    // units are in 32MB
                    driveInfo->totalWritesToFlash =
                        (driveInfo->totalWritesToFlash * 1000 * 1000 * 32) / driveInfo->logicalSectorSize;
                }
                break;
            case 234: // Lifetime Write to Flash (SSD)
                if (ataCapabilities->seagateFamily == SEAGATE || (ataCapabilities->seagateFamily == SEAGATE_VENDOR_D ||
                                                                  ataCapabilities->seagateFamily == SEAGATE_VENDOR_E ||
                                                                  ataCapabilities->seagateFamily == SEAGATE_VENDOR_B))
                {
                    driveInfo->totalWritesToFlash = M_BytesTo8ByteValue(
                        0, currentAttribute.rawData[6], currentAttribute.rawData[5], currentAttribute.rawData[4],
                        currentAttribute.rawData[3], currentAttribute.rawData[2], currentAttribute.rawData[1],
            ...
            case 241: // Total Bytes written (SSD) Total LBAs written (HDD)
                if ((ataCapabilities->seagateFamily == SEAGATE || ataCapabilities->seagateFamily == SEAGATE_VENDOR_D ||
                     ...
                     ataCapabilities->seagateFamily == SEAGATE_VENDOR_G ||
                     ataCapabilities->seagateFamily == SEAGATE_VENDOR_K) &&
                    driveInfo->totalLBAsWritten == 0)
                {
                    driveInfo->totalLBAsWritten = M_BytesTo8ByteValue(
                        0, currentAttribute.rawData[6], currentAttribute.rawData[5], currentAttribute.rawData[4],
                        currentAttribute.rawData[3], currentAttribute.rawData[2], currentAttribute.rawData[1],
                        currentAttribute.rawData[0]);
                    if (ataCapabilities->seagateFamily == SEAGATE_VENDOR_D ||
                        ataCapabilities->seagateFamily == SEAGATE_VENDOR_E ||
                        ataCapabilities->seagateFamily == SEAGATE_VENDOR_B ||
                        ataCapabilities->seagateFamily == SEAGATE_VENDOR_F)
                    {
                        // some Seagate SSD's report this as GiB written, so convert to LBAs
                        driveInfo->totalLBAsWritten =
                            (driveInfo->totalLBAsWritten * 1024 * 1024 * 1024) / driveInfo->logicalSectorSize;
                    }
                    else if (ataCapabilities->seagateFamily == SEAGATE_VENDOR_K)
                    {
                        // units are 32MB, so convert to LBAs
                        driveInfo->totalLBAsWritten =
                            (driveInfo->totalLBAsWritten * 1000 * 1000 * 32) / driveInfo->logicalSectorSize;
                    }
                }
                break;
            case 242: // Total Bytes read (SSD) Total LBAs read (HDD)
                if ((ataCapabilities->seagateFamily == SEAGATE || ataCapabilities->seagateFamily == SEAGATE_VENDOR_D ||
                     ...
                     ataCapabilities->seagateFamily == SEAGATE_VENDOR_G ||
                     ataCapabilities->seagateFamily == SEAGATE_VENDOR_K) &&
                    driveInfo->totalLBAsRead == 0)
                {
                    driveInfo->totalLBAsRead = M_BytesTo8ByteValue(
                        0, currentAttribute.rawData[6], currentAttribute.rawData[5], currentAttribute.rawData[4],
                        currentAttribute.rawData[3], currentAttribute.rawData[2], currentAttribute.rawData[1],
                        currentAttribute.rawData[0]);
                    if (ataCapabilities->seagateFamily == SEAGATE_VENDOR_D ||
                        ...
                        ataCapabilities->seagateFamily == SEAGATE_VENDOR_F)
                    {
                        ...
                        driveInfo->totalLBAsRead =
                            (driveInfo->totalLBAsRead * 1024 * 1024 * 1024) / driveInfo->logicalSectorSize;
                    }
                    else if (ataCapabilities->seagateFamily == SEAGATE_VENDOR_K)
                    {
                        // units are 32MB, so convert to LBAs
                        driveInfo->totalLBAsRead =
                            (driveInfo->totalLBAsRead * 1000 * 1000 * 32) / driveInfo->logicalSectorSize;
                    }
                }
                break;
            default:
                                                                  ...
                                                                  uint32_t              dataLength)
{
    ...
    if (device && info && securityProtocolList && dataLength > 8)
    {
        ...
        uint32_t bufIter = UINT32_C(8);
        // Check endianness. ATA is SUPPOSED to report little endian and SCSI/NVMe report big endian, but that doesn't
        // seem like it's always followed.
        ...
        //       This is necessary for any security protocol 0 (information) buffers as I've seen all kinds of weird
        //       combinations - TJE
        // For now, just take the shorter value of the two lengths given....seems true for everything I've encountered
        // so far.
        ...
        for (; (bufIter - 8) < length && (bufIter - 8) < dataLength; bufIter++)
        {
            ...
            case SECURITY_PROTOCOL_ATA_DEVICE_SERVER_PASSWORD:
            {
                ...
                if (SUCCESS == scsi_SecurityProtocol_In(device, SECURITY_PROTOCOL_ATA_DEVICE_SERVER_PASSWORD, 0, false,
                                                        16, ataSecurityInfo))
                {

                    info->ataSecurityInfo.securityEraseUnitTimeMinutes =
                        M_BytesTo2ByteValue(ataSecurityInfo[2], ataSecurityInfo[3]);
                    info->ataSecurityInfo.enhancedSecurityEraseUnitTimeMinutes =
                        M_BytesTo2ByteValue(ataSecurityInfo[4], ataSecurityInfo[5]);
                    info->ataSecurityInfo.masterPasswordIdentifier =
                        M_BytesTo2ByteValue(ataSecurityInfo[6], ataSecurityInfo[7]);
                    // check the bits now
...
eReturnValues get_ATA_Drive_Information(const tDevice* device, ptrDriveInformationSAS_SATA driveInfo)
{
    ...
    if ((ataCap.gplSupported &&
         SUCCESS == send_ATA_Read_Log_Ext_Cmd(device, ATA_LOG_DIRECTORY, 0, logBuffer, logBufferSize, 0)) ||
        (ataCap.smartErrorLoggingSupported &&
         SUCCESS == ata_SMART_Read_Log(device, ATA_LOG_DIRECTORY, logBuffer, logBufferSize)))
    {
    ...
    if (gotLogDirectory || ataCap.smartErrorLoggingSupported)
    {
        ...
        uint32_t devStatsSize         = UINT32_C(0);
        uint32_t idDataLogSize        = UINT32_C(0);
        uint32_t hybridInfoSize       = UINT32_C(0);
        uint32_t smartSelfTest        = UINT32_C(0);
        uint32_t extSelfTest          = UINT32_C(0);
        uint32_t hostlogging          = UINT32_C(0);
        uint32_t sctStatus            = UINT32_C(0);
        uint32_t concurrentRangesSize = UINT32_C(0);
        ...
        else
        {
            ...
            // The only other logs to look at in here are SMART error log (summary and comprehensive) and selective self
            // test
        ...
        if (idDataLogSize > 0)
        {
            ...
            if (idDataLog != M_NULLPTR)
            {
                if (SUCCESS == get_ATA_Log(device, ATA_LOG_IDENTIFY_DEVICE_DATA, M_NULLPTR, M_NULLPTR, true, true, true,
                                           idDataLog, idDataLogSize, M_NULLPTR, 0, 0))
                {
        ...
        if (devStatsSize > 0) // can come from GPL or SMART
        {
            ...
            if (devStats != M_NULLPTR)
            {
                if (SUCCESS == get_ATA_Log(device, ATA_LOG_DEVICE_STATISTICS, M_NULLPTR, M_NULLPTR, true, true, true,
                                           devStats, devStatsSize, M_NULLPTR, 0, 0))
                {
                    // call function to fill in data from ID data log
        ...
        {
            if (SUCCESS ==
                send_ATA_Read_Log_Ext_Cmd(device, ATA_LOG_HYBRID_INFORMATION, 0, logBuffer, LEGACY_DRIVE_SEC_SIZE, 0))
            {
                driveInfo->hybridNANDSize =
                    M_BytesTo8ByteValue(logBuffer[23], logBuffer[22], logBuffer[21], logBuffer[20], logBuffer[19],
                                        logBuffer[18], logBuffer[17], logBuffer[16]) *
        ...
        if (ataCap.sctSupported && sctStatus > 0) // GPL or SMART
        {
                                       ...
                                       logBuffer, ATA_LOG_PAGE_LEN_BYTES, M_NULLPTR, 0, 0))
            {
                ...
                if (sctFormatVersion > 1) // cannot find spec for revision 1 of this log, but we'll keep this safe until
                                          // I find it with this check
                {
        ...
        if (ataCap.gplSupported && farmLogSize)
        {
            ...
            if (farmData != M_NULLPTR)
            {
                ...
                if (SUCCESS == send_ATA_Read_Log_Ext_Cmd(device, SEAGATE_ATA_LOG_FIELD_ACCESSIBLE_RELIABILITY_METRICS,
                                                         0, farmData, 16384, 0))
                {
                    uint64_t farmSignature = M_BytesTo8ByteValue(farmData[7], farmData[6], farmData[5], farmData[4],
                                                                 farmData[3], farmData[2], farmData[1], farmData[0]);
                    if (farmSignature & BIT63 && farmSignature & BIT62 &&
                        (farmSignature & UINT64_C(0x00FFFFFFFFFFFF)) == SEAGATE_FARM_LOG_SIGNATURE)
                    {
                        add_Feature_To_Supported_List(driveInfo->featuresSupported,
                                                      &driveInfo->numberOfFeaturesSupported,
                        ...
                        if (SUCCESS ==
                            send_ATA_Read_Log_Ext_Cmd(device, SEAGATE_ATA_LOG_FIELD_ACCESSIBLE_RELIABILITY_METRICS,
                                                      (UINT16_C(16384) / LEGACY_DRIVE_SEC_SIZE), farmData, 16384, 0))
                        {
                            // verify page number (first 8 bytes)
                            uint64_t farmPage1ID =
                                M_BytesTo8ByteValue(farmData[7], farmData[6], farmData[5], farmData[4], farmData[3],
                                                    farmData[2], farmData[1], farmData[0]);
                            if (farmSignature & BIT63 && farmSignature & BIT62 &&
                                (farmPage1ID & UINT64_C(0x00FFFFFFFFFFFF)) == 1)
                            {
                                uint64_t dateOfManufactureQWord =
                                    M_BytesTo8ByteValue(farmData[367], farmData[366], farmData[365], farmData[364],
                                                        farmData[363], farmData[362], farmData[361], farmData[360]);
                                if (dateOfManufactureQWord & BIT63 &&
                                    dateOfManufactureQWord & BIT62) // supported and valid info
                                {
                                    ...
                                    if (!get_And_Validate_Integer_Input_Uint8(domWeekStr, NULL, ALLOW_UNIT_NONE,
                                                                              &driveInfo->manufactureWeek))
                                    {
                                        driveInfo->dateOfManufactureValid = false;
                                    }
                                    if (!get_And_Validate_Integer_Input_Uint16(domYearStr, NULL, ALLOW_UNIT_NONE,
        ...
        else
        {
            ...
            if (protocolList)
            {
                if (SUCCESS == ata_Trusted_Receive(device, device->drive_info.ata_Options.dmaSupported, 0, 0,
                                                   protocolList, LEGACY_DRIVE_SEC_SIZE))
                {
                    if (SUCCESS == get_Security_Features_From_Security_Protocol(device, &driveInfo->securityInfo,
                                                                                protocolList, LEGACY_DRIVE_SEC_SIZE))
                    {
                        if (driveInfo->securityInfo.tcg)
                        {
                            ...
                            add_Feature_To_Supported_List(driveInfo->featuresSupported,
                                                          &driveInfo->numberOfFeaturesSupported, "TCG");
                        }
                        if (driveInfo->securityInfo.scsa)
                        {
                            add_Feature_To_Supported_List(driveInfo->featuresSupported,
                                                          &driveInfo->numberOfFeaturesSupported, "SCSA");
                        }
                        if (driveInfo->securityInfo.ieee1667)
                        {
                            add_Feature_To_Supported_List(driveInfo->featuresSupported,
                                                          &driveInfo->numberOfFeaturesSupported, "IEEE 1667");
                        }
                    }
                }
...
typedef struct s_scsiIdentifyInfo
{
    uint8_t version;
                                           ...
                                           uint32_t                    dataLength)
{
    ...
    if (driveInfo && scsiInfo && inquiryData && dataLength >= INQ_RETURN_DATA_LENGTH_SCSI2)
    {
        ...
        case 0:
            /*add_Specification_To_Supported_List(driveInfo->specificationsSupported,
            &driveInfo->numberOfSpecificationsSupported, "Does not conform to a SCSI standard");

            break;*/
            // Note: Old SCSI/SPC standards may report multiple specifications supported in this byte
            // Codes 08-0Ch, 40-44h, 48-4Ch, & 88h-8Ch are skipped in there. These were various ways to report
            // ISO/ECMA/ANSI standard conformance seperately for the same SCSI-x specification. New standards (SPC6 or
            // whatever is next) may use these values and they should be used.
        case 0x81:
        case 0x01:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "SCSI");
            scsiInfo->version = 1;
            break;
        case 0x02:
        case 0x80:
        case 0x82:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "SCSI-2");
            scsiInfo->version = 2;
            break;
        case 0x83:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "SPC");
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "SCSI-2");
            scsiInfo->version = 3;
            break;
        case 0x84:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "SPC-2");
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "SCSI-2");
            scsiInfo->version = 4;
            break;
        ...
        case 0x05:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "SPC-3");
            break;
        case 0x06:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "SPC-4");
            break;
        case 0x07:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "SPC-5");
            break;
        default:
            break;
        }
        if (responseFormat == 1)
        {
            // response format of 1 means there is compliance with the Common Command Set specification, which is
            // partial SCSI2 support.
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                                &driveInfo->numberOfSpecificationsSupported, "CCS");
        }
        ...
        if (scsiInfo->version >= 4 && (inquiryData[4] + 4) > 57)
        {
            ...
            uint16_t versionDescriptor = UINT16_C(0);
            uint8_t  versionIter       = UINT8_C(0);
            for (; versionIter < INQ_MAX_VERSION_DESCRIPTORS; versionIter++)
            {
                versionDescriptor = 0;
                versionDescriptor =
                    M_BytesTo2ByteValue(inquiryData[(versionIter * 2) + 58], inquiryData[(versionIter * 2) + 59]);
                                       ...
                                       ptrSCSIIdentifyInfo         scsiInfo)
{
    ...
    if (device && driveInfo && scsiInfo)
    {
        ...
        // some devices (external) don't support VPD pages or may have issue when trying to read them, so check if this
        // hack is set before attempting to read them
        if ((!device->drive_info.passThroughHacks.scsiHacks.noVPDPages ||
             device->drive_info.passThroughHacks.scsiHacks.unitSNAvailable) &&
            (scsiInfo->version >= 2 || device->drive_info.passThroughHacks.scsiHacks
                                           .unitSNAvailable)) // VPD pages indroduced in SCSI 2...also a USB hack
        {
            ...
            if (dummyUpVPDSupport)
            {
                    ...
                    device->drive_info.passThroughHacks.scsiHacks.noVPDPages)
                {
                    ...
                    // and/or a T10 string which isn't useful for the information being gathered in here today.
                ...
                else
                {
                    ...
                    tempBuf[offset] = UNIT_SERIAL_NUMBER; // SCSI2
                    ++offset;
                    if (scsiInfo->version >= SCSI_VERSION_SPC) // SPC
                    {
                    ...
                    {
                        tempBuf[offset] = ATA_INFORMATION; // SAT. Going to leave this in here no matter what other
                                                           // version info is available since SATLs needing this dummy
                                                           // data may support this regardless of other version info
                        ++offset;
                    }
                    if (scsiInfo->version >= SCSI_VERSION_SPC_4) // SBC3 - SPC4
                    {
                        if (scsiInfo->peripheralDeviceType == PERIPHERAL_DIRECT_ACCESS_BLOCK_DEVICE ||
                            scsiInfo->peripheralDeviceType == PERIPHERAL_SIMPLIFIED_DIRECT_ACCESS_DEVICE ||
                    ...
                    // Add more pages to the dummy information as we need to. This may be useful to do in the future in
                    // case a device decides not to support a MANDATORY page or another page we care about
                }
                // set page length (n-3)
            ...
            uint8_t* supportedVPDPages =
                M_REINTERPRET_CAST(uint8_t*, safe_calloc(supportedVPDPagesLength, sizeof(uint8_t)));
            if (supportedVPDPages == M_NULLPTR)
            {
            ...
            for (vpdIter = 0;
                 vpdIter < supportedVPDPagesLength && !device->drive_info.passThroughHacks.scsiHacks.noVPDPages;
                 vpdIter++)
            {
                ...
                case UNIT_SERIAL_NUMBER:
                {
                                                ...
                                                supportedVPDPages[vpdIter], true, false))
                    {
                        ...
                        {
                            if (strncmp(driveInfo->vendorID, "SEAGATE", safe_strlen("SEAGATE")) == 0 &&
                                serialNumberLength == 0x14) // Check SEAGATE Vendor ID And check that the length matches
                ...
                case DEVICE_IDENTIFICATION:
                {
                                                ...
                                                DEVICE_IDENTIFICATION, true, false))
                    {
                        ...
                        if (devIDPageLen + 4 > INQ_RETURN_DATA_LENGTH)
                        {
                            ...
                            if (SUCCESS != scsi_Inquiry(device, deviceIdentification, devIDPageLen + 4,
                                                        DEVICE_IDENTIFICATION, true, false))
                            {
                        ...
                        // Below we loop through to the designator descriptors to find the WWN, and on SAS set the
                        // active port. we get the active phy from the low byte of the WWN when we find the association
                        // field set to 01b
                        uint64_t accotiatedWWN            = UINT64_C(0);
                        uint8_t  association              = UINT8_C(0);
                        uint32_t deviceIdentificationIter = UINT32_C(4);
                        uint16_t pageLength = M_BytesTo2ByteValue(deviceIdentification[2], deviceIdentification[3]);
                        uint8_t  designatorLength   = UINT8_C(0);
                        uint8_t  protocolIdentifier = UINT8_C(0);
                        uint8_t  designatorType     = UINT8_C(0);
                        for (; deviceIdentificationIter < C_CAST(uint32_t, pageLength + UINT16_C(4));
                             deviceIdentificationIter += designatorLength)
                        {
                            ...
                            case 0: // associated with the addressed logical unit
                                ...
                                {
                                    driveInfo->worldWideNameSupported = true;
                                    // all NAA values other than 6 currently are 8 bytes long, so get all 8 of those
                                    // bytes in.
                                    driveInfo->worldWideName =
                                        M_BytesTo8ByteValue(deviceIdentification[deviceIdentificationIter + 4],
                                                            deviceIdentification[deviceIdentificationIter + 5],
                                                            deviceIdentification[deviceIdentificationIter + 6],
                                                            deviceIdentification[deviceIdentificationIter + 7],
                                                            deviceIdentification[deviceIdentificationIter + 8],
                                                            deviceIdentification[deviceIdentificationIter + 9],
                                                            deviceIdentification[deviceIdentificationIter + 10],
                                                            deviceIdentification[deviceIdentificationIter + 11]);
                                    ...
                                    if (naa == 6)
                                    {
                                        ...
                                        driveInfo->worldWideNameExtension =
                                            M_BytesTo8ByteValue(deviceIdentification[deviceIdentificationIter + 12],
                                                                deviceIdentification[deviceIdentificationIter + 13],
                                                                deviceIdentification[deviceIdentificationIter + 14],
                                                                deviceIdentification[deviceIdentificationIter + 15],
                                                                deviceIdentification[deviceIdentificationIter + 16],
                                                                deviceIdentification[deviceIdentificationIter + 17],
                                                                deviceIdentification[deviceIdentificationIter + 18],
                                                                deviceIdentification[deviceIdentificationIter + 19]);
                                    }
                                }
                                break;
                            case 1: // associated with the target port that received the command
                                ...
                                {
                                    if (protocolIdentifier == 0x06 &&
                                        designatorType == 0x03) // SAS->only place that getting a port number makes
                                                                // sense right now since we aren't gathering port speed
                                                                // for other interfaces since it isn't reported.
                                    {
                                        // we know we have found the right designator, so read the WWN, and check the
                                        // lowest nibble for the port number
                                        accotiatedWWN =
                                            M_BytesTo8ByteValue(deviceIdentification[deviceIdentificationIter + 4],
                                                                deviceIdentification[deviceIdentificationIter + 5],
                                                                deviceIdentification[deviceIdentificationIter + 6],
                ...
                case EXTENDED_INQUIRY_DATA:
                {
                    ...
                    if (SUCCESS == scsi_Inquiry(device, extendedInquiryData, VPD_EXTENDED_INQUIRY_LEN,
                                                EXTENDED_INQUIRY_DATA, true, false))
                    {
                            ...
                            // read supported lengths and protection types VPD page
                            {
                                ...
                                if (supportedBlockSizesAndProtectionTypes != M_NULLPTR)
                                {
                                    if (SUCCESS == scsi_Inquiry(device, supportedBlockSizesAndProtectionTypes,
                                                                ...
                                                                SUPPORTED_BLOCK_LENGTHS_AND_PROTECTION_TYPES, true,
                                                                false))
                                    {
                                        supportedBlockSizesAndProtectionTypesLength =
                                            M_BytesTo2ByteValue(supportedBlockSizesAndProtectionTypes[2],
                                                                supportedBlockSizesAndProtectionTypes[3]);
                                        uint8_t* temp = C_CAST(
                                            uint8_t*,
                                                                    ...
                                                                    false))
                                        {
                                            ...
                                            for (uint32_t offset = UINT16_C(4);
                                                 offset < C_CAST(uint32_t, supportedBlockSizesAndProtectionTypesLength +
                                                                               UINT16_C(4));
                                                 offset += UINT16_C(8))
                                            {
                                                ...
                                                if (scsiInfo->protectionType1Supported &&
                                                    scsiInfo->protectionType2Supported &&
                                                    scsiInfo->protectionType3Supported)
                                                {
                ...
                case POWER_CONDITION:
                    ...
                    // add_Feature_To_Supported_List(driveInfo->featuresSupported,
                    // &driveInfo->numberOfFeaturesSupported, "EPC");
                    break;
                ...
                case LOGICAL_BLOCK_PROVISIONING:
                {
                                                ...
                                                LOGICAL_BLOCK_PROVISIONING, true, false))
                    {
                        if (logicalBlockProvisioning[5] & BIT7)
                        {
                            ...
                            {
                                add_Feature_To_Supported_List(driveInfo->featuresSupported,
                                                              &driveInfo->numberOfFeaturesSupported, "UNMAP");
                            }
                        }
                    }
                ...
                case BLOCK_LIMITS:
                {
                    ...
                    if (SUCCESS == scsi_Inquiry(device, blockLimits, VPD_BLOCK_LIMITS_LEN, BLOCK_LIMITS, true, false))
                    {
                        ...
                        uint32_t maxAtomicLen =
                            M_BytesTo4ByteValue(blockLimits[44], blockLimits[45], blockLimits[46], blockLimits[47]);
                        uint32_t atomicAlign =
                            M_BytesTo4ByteValue(blockLimits[48], blockLimits[49], blockLimits[50], blockLimits[51]);
                        uint32_t atomicXferLenGran =
                            M_BytesTo4ByteValue(blockLimits[52], blockLimits[53], blockLimits[54], blockLimits[55]);
                        uint32_t maxAtomicLenWAtomicBoundary =
                            M_BytesTo4ByteValue(blockLimits[56], blockLimits[57], blockLimits[58], blockLimits[59]);
                        uint32_t maxAtomicBoundarySize =
                            M_BytesTo4ByteValue(blockLimits[60], blockLimits[61], blockLimits[62], blockLimits[63]);
                        if (writeSameLength > 0)
                        {
                            add_Feature_To_Supported_List(driveInfo->featuresSupported,
                                                          &driveInfo->numberOfFeaturesSupported, "Write Same");
                        }
                        if (maxAtomicLen > 0 || atomicAlign > 0 || atomicXferLenGran > 0 ||
                            maxAtomicLenWAtomicBoundary > 0 || maxAtomicBoundarySize > 0)
                        {
                            add_Feature_To_Supported_List(driveInfo->featuresSupported,
                                                          &driveInfo->numberOfFeaturesSupported, "Atomic Writes");
                        }
                    }
                ...
                case ATA_INFORMATION:
                {
                    ...
                    if (SUCCESS ==
                        scsi_Inquiry(device, ataInformation, VPD_ATA_INFORMATION_LEN, ATA_INFORMATION, true, false))
                    {
                        add_Feature_To_Supported_List(driveInfo->featuresSupported,
                ...
                {
                    uint32_t concurrentRangesLength =
                        (15 * 32) +
                    ...
                    if (SUCCESS == scsi_Inquiry(device, concurrentRangesData, concurrentRangesLength,
                                                CONCURRENT_POSITIONING_RANGES, true, false))
                    {
                        // calculate how many ranges are being reported by the device.
                        driveInfo->concurrentPositioningRanges = C_CAST(
                            uint8_t, (M_BytesTo2ByteValue(concurrentRangesData[2], concurrentRangesData[3]) - 60) /
                ...
                case ZONED_BLOCK_DEVICE_CHARACTERISTICS:
                {
                    ...
                    if (SUCCESS == scsi_Inquiry(device, zbdCharacteristics, VPD_ZONED_BLOCK_DEVICE_CHARACTERISTICS_LEN,
                                                ZONED_BLOCK_DEVICE_CHARACTERISTICS, true, false))
                    {
                                       ...
                                       ptrSCSIIdentifyInfo         scsiInfo)
{
    ...
    if (device && driveInfo && scsiInfo)
    {
        ...
        if (scsiInfo->version >= 2 && scsiInfo->peripheralDeviceType != PERIPHERAL_SIMPLIFIED_DIRECT_ACCESS_DEVICE &&
            !device->drive_info.passThroughHacks.scsiHacks.noLogPages) // SCSI2 introduced log pages
        {
            ...
            if (scsiLogBuf != M_NULLPTR)
            {
                if (!device->drive_info.passThroughHacks.scsiHacks.noLogSubPages &&
                    SUCCESS != scsi_Log_Sense_Cmd(device, false, LPC_CUMULATIVE_VALUES,
                                                  LP_SUPPORTED_LOG_PAGES_AND_SUBPAGES, 0xFF, 0, scsiLogBuf, 512))
                {
                    // either device doesn't support logs, or it just doesn't support subpages, so let's try reading the
                    // list of supported pages (no subpages) before saying we need to dummy up the list
                    if (SUCCESS != scsi_Log_Sense_Cmd(device, false, LPC_CUMULATIVE_VALUES, LP_SUPPORTED_LOG_PAGES, 0,
                                                      0, scsiLogBuf, 512))
                    {
                ...
                {
                    // trying to read the list of supported pages can trigger this to show up due to invalid operation
                    // code when this happens, just return to save the time and effort.
                ...
                // this is really a work-around for USB drives since some DO support pages, but the don't actually list
                // them (same as the VPD pages above). Most USB drives don't work though - TJE
                if (dummyUpLogPages)
                {
                    uint16_t offset    = UINT16_C(4);
                    uint8_t  increment = UINT8_C(1); // change to 2 for subpages (spc4 added subpages)
                    ...
                    if (scsiInfo->peripheralDeviceType == PERIPHERAL_DIRECT_ACCESS_BLOCK_DEVICE ||
                        scsiInfo->peripheralDeviceType == PERIPHERAL_HOST_MANAGED_ZONED_BLOCK_DEVICE)
                    {
                        if (scsiInfo->version >= 6) // SBC3 is to be used in conjunction with SPC4. We may need to drop
                                                    // this one level later, but this should be ok
                        {
                    ...
                    {
                        scsiLogBuf[offset] =
                            LP_START_STOP_CYCLE_COUNTER; // just to be correct, we're not reading this today
                        offset += increment;
                    }
                    ...
                    if (scsiInfo->peripheralDeviceType == PERIPHERAL_DIRECT_ACCESS_BLOCK_DEVICE ||
                        scsiInfo->peripheralDeviceType == PERIPHERAL_HOST_MANAGED_ZONED_BLOCK_DEVICE)
                    {
                        if (scsiInfo->version >= 5) // SBC2 is to be used in conjunction with SPC3. We may need to drop
                                                    // this one level later, but this should be ok
                        {
                ...
                for (; logPageIter < M_Min(supportedPagesLength + LOG_PAGE_HEADER_LENGTH, LEGACY_DRIVE_SEC_SIZE) &&
                       !device->drive_info.passThroughHacks.scsiHacks.noLogPages;
                     logPageIter += incrementAmount)
                {
                    ...
                    case LP_WRITE_ERROR_COUNTERS:
                        if (subpageCode == 0)
                        {
                            ...
                            if (SUCCESS == scsi_Log_Sense_Cmd(device, false, LPC_CUMULATIVE_VALUES, pageCode,
                                                              subpageCode, 0x0005, writeErrorData, 16))
                            {
                                ...
                                if (M_BytesTo2ByteValue(writeErrorData[4], writeErrorData[5]) == 0x0005)
                                {
                                    ...
                                    case 1: // single byte
                                        ...
                                        break;
                                    case 2: // word
                                        driveInfo->totalBytesWritten =
                                            M_BytesTo2ByteValue(writeErrorData[8], writeErrorData[9]);
                                        break;
                                    case 4: // double word
                                        driveInfo->totalBytesWritten =
                                            M_BytesTo4ByteValue(writeErrorData[8], writeErrorData[9],
                                                                writeErrorData[10], writeErrorData[11]);
                                        break;
                                    case 8: // quad word
                                        driveInfo->totalBytesWritten = M_BytesTo8ByteValue(
                                            writeErrorData[8], writeErrorData[9], writeErrorData[10],
                                            writeErrorData[11], writeErrorData[12], writeErrorData[13],
                                            writeErrorData[14], writeErrorData[15]);
                                        break;
                                    ...
                                    {
                                        driveInfo->totalLBAsWritten =
                                            driveInfo->totalBytesWritten / driveInfo->logicalSectorSize;
                                    }
                                }
                            }
                    ...
                    case LP_READ_ERROR_COUNTERS:
                        if (subpageCode == 0)
                        {
                            ...
                            if (SUCCESS == scsi_Log_Sense_Cmd(device, false, LPC_CUMULATIVE_VALUES, pageCode,
                                                              subpageCode, 0x0005, readErrorData, 16))
                            {
                                if (M_BytesTo2ByteValue(readErrorData[4], readErrorData[5]) == 0x0005)
                                {
                                    ...
                                    case 1: // single byte
                                        ...
                                        break;
                                    case 2: // word
                                        driveInfo->totalBytesRead =
                                            M_BytesTo2ByteValue(readErrorData[8], readErrorData[9]);
                                        break;
                                    case 4: // double word
                                        driveInfo->totalBytesRead = M_BytesTo4ByteValue(
                                            readErrorData[8], readErrorData[9], readErrorData[10], readErrorData[11]);
                                        break;
                                    case 8: // quad word
                                        driveInfo->totalBytesRead = M_BytesTo8ByteValue(
                                            readErrorData[8], readErrorData[9], readErrorData[10], readErrorData[11],
                                            readErrorData[12], readErrorData[13], readErrorData[14], readErrorData[15]);
                                        break;
                                    default: // don't bother trying to read the data since it's in a more complicated
                                             // format to read than we care to handle in this code right now
                                        break;
                                    ...
                                    {
                                        driveInfo->totalLBAsRead =
                                            driveInfo->totalBytesRead / driveInfo->logicalSectorSize;
                                    }
                                }
                            }
                    ...
                    case LP_TEMPERATURE: // also environmental reporting
                        ...
                        case 0: // temperature
                        {
                            ...
                            if (SUCCESS == scsi_Log_Sense_Cmd(device, false, LPC_CUMULATIVE_VALUES, pageCode,
                                                              subpageCode, 0, temperatureData, 10))
                            {
                                driveInfo->temperatureData.temperatureDataValid = true;
                        ...
                        case 1: // environmental reporting
                        {
                            ...
                            if (SUCCESS == scsi_Log_Sense_Cmd(device, false, LPC_CUMULATIVE_VALUES, pageCode,
                                                              subpageCode, 0, environmentReporting, 16))
                            {
                                // NOTE: Lots of casts in here to avoid clang-tidy warnings and cert-c warnings about
                                // possible string conversion errors
                                //       To resolve in this case convert uint8_t to int8_t, then explicitly convert to
                                //       int16_t. -TJE
                                driveInfo->temperatureData.temperatureDataValid = true;
                                driveInfo->temperatureData.currentTemperature =
                                    M_STATIC_CAST(int16_t, M_STATIC_CAST(int8_t, environmentReporting[9]));
                                driveInfo->temperatureData.highestTemperature =
                                    M_STATIC_CAST(int16_t, M_STATIC_CAST(int8_t, environmentReporting[10]));
                                driveInfo->temperatureData.lowestTemperature =
                                    M_STATIC_CAST(int16_t, M_STATIC_CAST(int8_t, environmentReporting[11]));
                                driveInfo->temperatureData.highestValid = true;
                                driveInfo->temperatureData.lowestValid  = true;
                            }
                            // now get humidity data if available
                            if (SUCCESS == scsi_Log_Sense_Cmd(device, false, LPC_CUMULATIVE_VALUES, pageCode,
                                                              subpageCode, 0x0100, environmentReporting, 16))
                            {
                    ...
                    case LP_UTILIZATION: // also start-stop cycle counter
                        ...
                        case 0x00: // start-stop cycle count
                        {
                            ...
                            if (SUCCESS == scsi_Log_Sense_Cmd(device, false, LPC_CUMULATIVE_VALUES, pageCode,
                                                              subpageCode, 0x0001, startStopCounterLog, 14))
                            {
                                    ...
                                    startStopCounterLog[1] == 0)
                                {
                                    ...
                                    // but did not give this parameter code
                                    if (M_BytesTo2ByteValue(startStopCounterLog[4], startStopCounterLog[5]) == 0x0001)
                                    {
                                        // DOM found
                                        char domWeekStr[3]                = {C_CAST(char, startStopCounterLog[12]),
                                                                             C_CAST(char, startStopCounterLog[13]), 0};
                                        char domYearStr[5]                = {C_CAST(char, startStopCounterLog[8]),
                                                                             C_CAST(char, startStopCounterLog[9]),
                                                                             C_CAST(char, startStopCounterLog[10]),
                                                                             C_CAST(char, startStopCounterLog[11]), 0};
                                        driveInfo->dateOfManufactureValid = true;
                                        if (!get_And_Validate_Integer_Input_Uint8(domWeekStr, NULL, ALLOW_UNIT_NONE,
                                                                                  &driveInfo->manufactureWeek))
                                        {
                        ...
                        case 0x01: // utilization
                        {
                            ...
                            if (SUCCESS == scsi_Log_Sense_Cmd(device, false, LPC_CUMULATIVE_VALUES, pageCode,
                                                              subpageCode, 0, utilizationData, 10))
                            {
                                // bytes 9 & 10
                                driveInfo->deviceReportedUtilizationRate =
                                    C_CAST(double, M_BytesTo2ByteValue(utilizationData[8], utilizationData[9])) /
                                    1000.0;
                            }
                    ...
                    case LP_APPLICATION_CLIENT:
                        ...
                        case 0x00: // application client
                        {
                            ...
                            if (SUCCESS == scsi_Log_Sense_Cmd(device, false, LPC_CUMULATIVE_VALUES, pageCode,
                                                              subpageCode, 0, applicationClient, 4))
                            {
                    ...
                    case LP_SELF_TEST_RESULTS:
                        if (subpageCode == 0)
                        {
                            ...
                            if (SUCCESS == scsi_Log_Sense_Cmd(device, false, LPC_CUMULATIVE_VALUES, pageCode,
                                                              subpageCode, 0, selfTestResults,
                                                              LP_SELF_TEST_RESULTS_LEN))
                            {
                                uint8_t parameterOffset = UINT8_C(4);
                                add_Feature_To_Supported_List(driveInfo->featuresSupported,
                                                              &driveInfo->numberOfFeaturesSupported, "Self Test");
                                ...
                                driveInfo->dstInfo.powerOnHours = M_BytesTo2ByteValue(
                                    selfTestResults[parameterOffset + 6], selfTestResults[parameterOffset + 7]);
                                driveInfo->dstInfo.errorLBA = M_BytesTo8ByteValue(
                                    selfTestResults[parameterOffset + 8], selfTestResults[parameterOffset + 9],
                                    selfTestResults[parameterOffset + 10], selfTestResults[parameterOffset + 11],
                                    selfTestResults[parameterOffset + 12], selfTestResults[parameterOffset + 13],
                                    selfTestResults[parameterOffset + 14], selfTestResults[parameterOffset + 15]);
                            }
                    ...
                    case LP_SOLID_STATE_MEDIA:
                        if (subpageCode == 0)
                        {
                            ...
                            if (SUCCESS == scsi_Log_Sense_Cmd(device, false, LPC_CUMULATIVE_VALUES, pageCode,
                                                              subpageCode, 0x0001, ssdEnduranceData, 12))
                            {
                    ...
                    case LP_BACKGROUND_SCAN_RESULTS:
                        if (subpageCode == 0)
                        {
                            ...
                            if (SUCCESS == scsi_Log_Sense_Cmd(device, false, LPC_CUMULATIVE_VALUES, pageCode,
                                                              subpageCode, 0, backgroundScanResults, 19))
                            {
                                // bytes 8 to 11
                                driveInfo->powerOnMinutes =
                                    M_BytesTo4ByteValue(backgroundScanResults[8], backgroundScanResults[9],
                                                        backgroundScanResults[10], backgroundScanResults[11]);
                                driveInfo->powerOnMinutesValid = true;
                            }
                    ...
                    case LP_GENERAL_STATISTICS_AND_PERFORMANCE:
                        if (subpageCode == 0)
                        {
                            ...
                            if (SUCCESS == scsi_Log_Sense_Cmd(device, false, LPC_CUMULATIVE_VALUES, pageCode,
                                                              subpageCode, 0x0001, generalStatsAndPerformance, 72))
                            {
                                // total lbas written (number of logical blocks received)
                                driveInfo->totalLBAsWritten =
                                    M_BytesTo8ByteValue(generalStatsAndPerformance[24], generalStatsAndPerformance[25],
                                                        generalStatsAndPerformance[26], generalStatsAndPerformance[27],
                                                        generalStatsAndPerformance[28], generalStatsAndPerformance[29],
                                                        generalStatsAndPerformance[30], generalStatsAndPerformance[31]);
                                // convert to bytes written
                                driveInfo->totalBytesWritten =
                                    driveInfo->totalLBAsWritten * driveInfo->logicalSectorSize;
                                // total lbas read (number of logical blocks transmitted)
                                driveInfo->totalLBAsRead =
                                    M_BytesTo8ByteValue(generalStatsAndPerformance[32], generalStatsAndPerformance[33],
                                                        generalStatsAndPerformance[34], generalStatsAndPerformance[35],
                                                        generalStatsAndPerformance[36], generalStatsAndPerformance[37],
                                                        generalStatsAndPerformance[38], generalStatsAndPerformance[39]);
                                // convert to bytes written
                                driveInfo->totalBytesRead = driveInfo->totalLBAsRead * driveInfo->logicalSectorSize;
                            }
                    ...
                    case LP_INFORMATION_EXCEPTIONS:
                        if (subpageCode == 0)
                        {
                            ...
                            if (SUCCESS == scsi_Log_Sense_Cmd(device, false, LPC_CUMULATIVE_VALUES, pageCode,
                                                              subpageCode, 0, informationExceptions, 11))
                            {
                                ...
                                {
                                    // temperature log page was not read, neither was environmental reporting, but we do
                                    // have a current temperature reading here, so use it
                                    driveInfo->temperatureData.temperatureDataValid = true;
                    ...
                    case SEAGATE_LP_FARM:
                        if (subpageCode == SEAGATE_FARM_SP_CURRENT)
                        {
                            ...
                            if (SUCCESS == scsi_Log_Sense_Cmd(device, false, LPC_CUMULATIVE_VALUES, pageCode,
                                                              subpageCode, 0, farmData, 76))
                            {
                                uint64_t farmSignature =
                                    M_BytesTo8ByteValue(farmData[4], farmData[5], farmData[6], farmData[7], farmData[8],
                                                        farmData[9], farmData[10], farmData[11]);
                                if (farmSignature & BIT63 && farmSignature & BIT62 &&
                                    (farmSignature & UINT64_C(0x00FFFFFFFFFFFF)) == SEAGATE_FARM_LOG_SIGNATURE)
                                {
                                    add_Feature_To_Supported_List(driveInfo->featuresSupported,
                                ...
                                // NOTE: If for any reason DOM was not already read from standard page, can read it here
                                // too
                            }
                            safe_free_aligned(&farmData);
                        }
                        break;
                    case 0x3C: // Vendor specific page. we're checking this page on Seagate drives for an enhanced usage
                        ...
                        if (is_Seagate_Family(device) == SEAGATE || is_Seagate_Family(device) == SEAGATE_VENDOR_A)
                        {
                            ...
                            if (SUCCESS == scsi_Log_Sense_Cmd(device, false, LPC_CUMULATIVE_VALUES, pageCode, 0, 0x8004,
                                                              ssdUsage, 12))
                            {
                                driveInfo->percentEnduranceUsed =
                                    (C_CAST(double,
                                            M_BytesTo4ByteValue(ssdUsage[8], ssdUsage[9], ssdUsage[10], ssdUsage[11])) /
                                     1000000.00) *
                                    100.00;
                            }
                                        ...
                                        ptrSCSIIdentifyInfo         scsiInfo)
{
    ...
    if (device && driveInfo && scsiInfo)
    {
        ...
        {
            uint16_t numberOfPages = UINT16_C(0);
            uint16_t offset        = UINT16_C(0);
            // create a list of mode pages (and any subpages) we care about reading and go through that list reading
            // each one
            DECLARE_ZERO_INIT_ARRAY(
                uint8_t, listOfModePagesAndSubpages,
                512); // allow 10 entries in the list...update the loop condition below if this is adjusted
                ...
                device->drive_info.interface_type != MMC_INTERFACE && device->drive_info.interface_type != SD_INTERFACE)
            {
                if (driveInfo->rotationRate == 0 &&
                    (scsiInfo->peripheralDeviceType == PERIPHERAL_DIRECT_ACCESS_BLOCK_DEVICE ||
                     ...
                     scsiInfo->peripheralDeviceType == PERIPHERAL_SIMPLIFIED_DIRECT_ACCESS_DEVICE))
                {
                    listOfModePagesAndSubpages[offset] =
                        MP_RIGID_DISK_GEOMETRY; // To get medium rotation rate if we didn't already get it. This page is
                                                // long obsolete
                    offset += 2;
                }
            }
                ...
                scsiInfo->version >= SCSI_VERSION_SPC_3) // SPC3 added subpage codes
            {
                    ...
                    device->drive_info.interface_type != SD_INTERFACE)
                {
                    ...
                    listOfModePagesAndSubpages[offset] =
                        MP_CONTROL; // IO Advice Hints (can we read this page or not basically)
                ...
                device->drive_info.interface_type != MMC_INTERFACE && device->drive_info.interface_type != SD_INTERFACE)
            {
                ...
                if (!device->drive_info.passThroughHacks.scsiHacks.noModeSubPages &&
                    scsiInfo->version >= 5) // SPC3 added subpage codes
                {
                ...
                scsiInfo->version >= SCSI_VERSION_SPC_3) // SPC3 added subpage codes
            {
                ...
                listOfModePagesAndSubpages[offset + 1] =
                    0xF1; // reading this for the ATA APM settings (check if supported really)
                offset += 2;
            }
            if (scsiInfo->version >= SCSI_VERSION_SPC) // Added in SPC
            {
                listOfModePagesAndSubpages[offset] =
                    MP_INFORMATION_EXCEPTIONS_CONTROL; // SMART/informational exceptions & MRIE value. Dexcept?
                                                       // Warnings?
                listOfModePagesAndSubpages[offset + 1] = 0;
                offset += 2;
            }
            ...
            uint16_t modeIter           = UINT16_C(0);
            uint8_t  protocolIdentifier = UINT8_C(0);
            for (uint16_t pageCounter = UINT16_C(0); modeIter < offset && pageCounter < numberOfPages &&
                                                     !device->drive_info.passThroughHacks.scsiHacks.noModePages;
                 modeIter += 2, ++pageCounter)
            {
                ...
                case MP_READ_WRITE_ERROR_RECOVERY:
                    ...
                    {
                    case 0:
                        // check if AWRE and ARRE are supported or can be changed before checking if they are enabled or
                        // not.
                        {
                            char*    awreString       = M_NULLPTR;
                            char*    arreString       = M_NULLPTR;
                            uint32_t awreStringLength = UINT32_C(0);
                            uint32_t arreStringLength = UINT32_C(0);
                            DECLARE_ZERO_INIT_ARRAY(
                                uint8_t, readWriteErrorRecovery,
                                12 + SCSI_MODE_PAGE_MIN_HEADER_LENGTH); // need to include header length in this
                            bool     pageRead     = false;
                            bool     defaultsRead = false;
                            bool     sixByte      = false;
                            uint16_t headerLength = UINT16_C(0);
                            uint16_t modeDataLen  = UINT16_C(0);
                            if (SUCCESS == get_SCSI_Mode_Page(device, MPC_DEFAULT_VALUES, pageCode, subPageCode,
                                                              M_NULLPTR, M_NULLPTR, true, readWriteErrorRecovery,
                                                              12 + SCSI_MODE_PAGE_MIN_HEADER_LENGTH, M_NULLPTR,
                                                              &sixByte))
                            {
                                uint16_t blockDescLen = UINT16_C(0);
                                uint8_t  devSpecific  = UINT8_C(0);
                                defaultsRead          = true;

                                get_SBC_Mode_Header_Blk_Desc_Fields(sixByte, readWriteErrorRecovery,
                                                                    12 + SCSI_MODE_PAGE_MIN_HEADER_LENGTH, &modeDataLen,
                                                                    M_NULLPTR, &devSpecific, M_NULLPTR, &blockDescLen,
                                                                    M_NULLPTR, M_NULLPTR);
                                if (devSpecific & BIT7)
                                {
                            ...
                            if (defaultsRead && modeDataLen > 0)
                            {
                                // awre
                                if (readWriteErrorRecovery[headerLength + 2] & BIT7)
                                {
                                    if (awreString == M_NULLPTR)
                                    {
                                        awreStringLength = 30;
                                        awreString =
                                            M_REINTERPRET_CAST(char*, safe_calloc(awreStringLength, sizeof(char)));
                                    }
                                    else
                                    {
                                        awreStringLength = 30;
                                ...
                                {
                                    if (arreString == M_NULLPTR)
                                    {
                                        arreStringLength = 30;
                                        arreString =
                                            M_REINTERPRET_CAST(char*, safe_calloc(arreStringLength, sizeof(char)));
                                    }
                                    else
                                    {
                                        arreStringLength = 30;
                            ...
                            if (SUCCESS == get_SCSI_Mode_Page(device, MPC_CURRENT_VALUES, pageCode, subPageCode,
                                                              M_NULLPTR, M_NULLPTR, true, readWriteErrorRecovery,
                                                              12 + SCSI_MODE_PAGE_MIN_HEADER_LENGTH, M_NULLPTR,
                                                              &sixByte))
                            {
                ...
                case MP_RIGID_DISK_GEOMETRY:
                    ...
                    case 0:
                        if (driveInfo->rotationRate == 0)
                        {
                            ...
                            uint16_t modeDataLen  = UINT16_C(0);
                            if (SUCCESS == get_SCSI_Mode_Page(device, MPC_CURRENT_VALUES, pageCode, subPageCode,
                                                              M_NULLPTR, M_NULLPTR, true, rigidGeometry,
                                                              24 + SCSI_MODE_PAGE_MIN_HEADER_LENGTH, M_NULLPTR,
                                                              &sixByte))
                            {
                                uint16_t blockDescLen = UINT16_C(0);
                                uint8_t  devSpecific  = UINT8_C(0);
                                pageRead              = true;
                                get_SBC_Mode_Header_Blk_Desc_Fields(
                                    sixByte, rigidGeometry, 24 + SCSI_MODE_PAGE_MIN_HEADER_LENGTH, &modeDataLen,
                                    M_NULLPTR, &devSpecific, M_NULLPTR, &blockDescLen, M_NULLPTR, M_NULLPTR);
                                if (devSpecific & BIT7)
                                {
                                    driveInfo->isWriteProtected = true;
                                }
                            ...
                            if (pageRead && modeDataLen > 0)
                            {
                                driveInfo->rotationRate = M_BytesTo2ByteValue(rigidGeometry[headerLength + 20],
                                                                              rigidGeometry[headerLength + 21]);
                            }
                        }
                        break;
                    default:
                ...
                case MP_CACHING:
                    ...
                    case 0:
                    {
                        DECLARE_ZERO_INIT_ARRAY(
                            uint8_t, cachingPage,
                            20 + SCSI_MODE_PAGE_MIN_HEADER_LENGTH); // need to include header length in this
                        bool     pageRead     = false;
                        bool     sixByte      = false;
                        uint16_t headerLength = UINT16_C(0);
                        uint16_t modeDataLen  = UINT16_C(0);
                        if (SUCCESS == get_SCSI_Mode_Page(device, MPC_CURRENT_VALUES, pageCode, subPageCode, M_NULLPTR,
                                                          M_NULLPTR, true, cachingPage,
                                                          20 + SCSI_MODE_PAGE_MIN_HEADER_LENGTH, M_NULLPTR, &sixByte))
                        {
                            uint16_t blockDescLen = UINT16_C(0);
                            uint8_t  devSpecific  = UINT8_C(0);
                            pageRead              = true;
                            get_SBC_Mode_Header_Blk_Desc_Fields(
                                sixByte, cachingPage, 20 + SCSI_MODE_PAGE_MIN_HEADER_LENGTH, &modeDataLen, M_NULLPTR,
                                &devSpecific, M_NULLPTR, &blockDescLen, M_NULLPTR, M_NULLPTR);
                            if (devSpecific & BIT7)
                            {
                                driveInfo->isWriteProtected = true;
                            }
                        ...
                        {
                            // NV_DIS
                            driveInfo->nvCacheEnabled = !M_ToBool(
                                cachingPage[headerLength + 13] &
                                BIT0); // bit being set means disabled the cache, being set to 0 means cache is enabled.
                            ...
                            if (!driveInfo->writeCacheSupported || !driveInfo->readLookAheadSupported)
                            {
                                ...
                                if (SUCCESS == get_SCSI_Mode_Page(device, MPC_CHANGABLE_VALUES, pageCode, subPageCode,
                                                                  ...
                                                                  20 + SCSI_MODE_PAGE_MIN_HEADER_LENGTH, M_NULLPTR,
                                                                  &sixByte))
                                {
                                    uint16_t blockDescLen = UINT16_C(0);
                                    uint8_t  devSpecific  = UINT8_C(0);
                                    pageRead              = true;
                                    get_SBC_Mode_Header_Blk_Desc_Fields(
                                        sixByte, cachingPage, 20 + SCSI_MODE_PAGE_MIN_HEADER_LENGTH, &modeDataLen,
                                        M_NULLPTR, &devSpecific, M_NULLPTR, &blockDescLen, M_NULLPTR, M_NULLPTR);
                                    if (devSpecific & BIT7)
                                    {
                                        driveInfo->isWriteProtected = true;
                                ...
                                if (pageRead && modeDataLen > 0)
                                {
                                    // changable dictates if a bit can be changed or not. So unlike above where it is
                                    // indicating state, this indicates if it is supported or can be changed at all from
                                    // the current state
                ...
                case MP_CONTROL:
                    ...
                    case 0: // control mode page. No subpage
                    {
                        DECLARE_ZERO_INIT_ARRAY(
                            uint8_t, controlPage,
                            MP_CONTROL_LEN + SCSI_MODE_PAGE_MIN_HEADER_LENGTH); // need to include header length in this
                        bool     pageRead     = false;
                        bool     sixByte      = false;
                        uint16_t headerLength = UINT16_C(0);
                        uint16_t modeDataLen  = UINT16_C(0);
                        if (SUCCESS == get_SCSI_Mode_Page(device, MPC_CURRENT_VALUES, pageCode, subPageCode, M_NULLPTR,
                                                          M_NULLPTR, true, controlPage,
                                                          MP_CONTROL_LEN + SCSI_MODE_PAGE_MIN_HEADER_LENGTH, M_NULLPTR,
                                                          &sixByte))
                        {
                            uint16_t blockDescLen = UINT16_C(0);
                            uint8_t  devSpecific  = UINT8_C(0);
                            pageRead              = true;
                            get_SBC_Mode_Header_Blk_Desc_Fields(
                                sixByte, controlPage, MP_CONTROL_LEN + SCSI_MODE_PAGE_MIN_HEADER_LENGTH, &modeDataLen,
                                M_NULLPTR, &devSpecific, M_NULLPTR, &blockDescLen, M_NULLPTR, M_NULLPTR);
                            if (devSpecific & BIT7)
                            {
                                driveInfo->isWriteProtected = true;
                        ...
                        if (pageRead && modeDataLen > 0)
                        {
                            ...
                            if (get_bit_range_uint8(controlPage[headerLength + 0], 5, 0) == MP_CONTROL)
                            {
                                ...
                                {
                                    if (driveInfo->longDSTTimeMinutes ==
                                        0) // checking for zero since we may have already gotten this from the Extended
                                           // Inquiry VPD page
                                    {
                                        driveInfo->longDSTTimeMinutes =
                                            ((C_CAST(uint64_t, M_BytesTo2ByteValue(controlPage[headerLength + 10],
                                                                                   controlPage[headerLength + 11])) +
                                              UINT64_C(60)) -
                                             UINT64_C(1)) /
                                            UINT64_C(60); // rounding up to nearest minute
                                    }
                                }
                            }
                        }
                    ...
                    case 1: // controlExtension
                    {
                        ...
                        uint32_t dlcStringLength = UINT32_C(0);
                        DECLARE_ZERO_INIT_ARRAY(
                            uint8_t, controlExtensionPage,
                            MP_CONTROL_EXTENSION_LEN +
                                SCSI_MODE_PAGE_MIN_HEADER_LENGTH); // need to include header length in this
                        bool     pageRead     = false;
                        bool     defaultsRead = false;
                        bool     sixByte      = false;
                        uint16_t modeDataLen  = UINT16_C(0);
                        uint16_t headerLength = UINT16_C(0);
                        if (SUCCESS == get_SCSI_Mode_Page(device, MPC_DEFAULT_VALUES, pageCode, subPageCode, M_NULLPTR,
                                                          M_NULLPTR, true, controlExtensionPage,
                                                          MP_CONTROL_EXTENSION_LEN + SCSI_MODE_PAGE_MIN_HEADER_LENGTH,
                                                          M_NULLPTR, &sixByte))
                        {
                            uint16_t blockDescLen = UINT16_C(0);
                            uint8_t  devSpecific  = UINT8_C(0);
                            ...
                            get_SBC_Mode_Header_Blk_Desc_Fields(
                                sixByte, controlExtensionPage,
                                MP_CONTROL_EXTENSION_LEN + SCSI_MODE_PAGE_MIN_HEADER_LENGTH, &modeDataLen, M_NULLPTR,
                                &devSpecific, M_NULLPTR, &blockDescLen, M_NULLPTR, M_NULLPTR);
                            if (devSpecific & BIT7)
                            {
                                driveInfo->isWriteProtected = true;
                        ...
                        if (defaultsRead && modeDataLen > 0)
                        {
                            ...
                            {
                                if (dlcString == M_NULLPTR)
                                {
                                    dlcStringLength = 50;
                                    dlcString = M_REINTERPRET_CAST(char*, safe_calloc(dlcStringLength, sizeof(char)));
                                }
                                else
                                {
                                    dlcStringLength = 50;
                                    char* temp =
                                        M_REINTERPRET_CAST(char*, safe_reallocf(M_REINTERPRET_CAST(void**, &dlcString),
                                                                                dlcStringLength * sizeof(char)));
                                    if (temp != M_NULLPTR)
                                    {
                                        dlcString = temp;
                    ...
                    case 0x03: // CDL A
                        // case 0x04://CDL B
                        {
                            DECLARE_ZERO_INIT_ARRAY(
                                uint8_t, cdl,
                                36 + SCSI_MODE_PAGE_MIN_HEADER_LENGTH); // need to include header length in this
                            bool sixByte = false;
                            // uint16_t headerLength = UINT16_C(0);
                            if (SUCCESS == get_SCSI_Mode_Page(
                                               device, MPC_CURRENT_VALUES, pageCode, subPageCode, M_NULLPTR, M_NULLPTR,
                                               true, cdl, 36 + SCSI_MODE_PAGE_MIN_HEADER_LENGTH, M_NULLPTR, &sixByte))
                            {
                                add_Feature_To_Supported_List(driveInfo->featuresSupported,
                                                              &driveInfo->numberOfFeaturesSupported,
                                                              "Command Duration Limits");
                    ...
                    {
                        DECLARE_ZERO_INIT_ARRAY(
                            uint8_t, ioAdviceHints,
                            1040 + SCSI_MODE_PAGE_MIN_HEADER_LENGTH); // need to include header length in this
                        bool     pageRead     = false;
                        bool     sixByte      = false;
                        uint16_t headerLength = UINT16_C(0);
                        uint16_t modeDataLen  = UINT16_C(0);
                        if (SUCCESS == get_SCSI_Mode_Page(device, MPC_CURRENT_VALUES, pageCode, subPageCode, M_NULLPTR,
                                                          M_NULLPTR, true, ioAdviceHints,
                                                          1040 + SCSI_MODE_PAGE_MIN_HEADER_LENGTH, M_NULLPTR, &sixByte))
                        {
                            uint16_t blockDescLen = UINT16_C(0);
                            uint8_t  devSpecific  = UINT8_C(0);
                            pageRead              = true;
                            get_SBC_Mode_Header_Blk_Desc_Fields(
                                sixByte, ioAdviceHints, 1040 + SCSI_MODE_PAGE_MIN_HEADER_LENGTH, &modeDataLen,
                                M_NULLPTR, &devSpecific, M_NULLPTR, &blockDescLen, M_NULLPTR, M_NULLPTR);
                            if (devSpecific & BIT7)
                            {
                                driveInfo->isWriteProtected = true;
                        ...
                        if (pageRead && modeDataLen > 0)
                        {
                            // check if any of the Hints valid bits are set so we know it is enabled. TODO: add checking
                            // for the cache enabled bit?
                            bool valid = false;
                            for (uint32_t iter = headerLength + 15;
                                 iter < C_CAST(uint32_t, (UINT16_C(1040) + headerLength)); iter += 16)
                            {
                            ...
                            if (valid)
                            {
                                add_Feature_To_Supported_List(driveInfo->featuresSupported,
                                                              &driveInfo->numberOfFeaturesSupported,
                                                              "IO Advice Hints [Enabled]");
                            }
                            else
                            {
                                add_Feature_To_Supported_List(driveInfo->featuresSupported,
                                                              &driveInfo->numberOfFeaturesSupported, "IO Advice Hints");
                            }
                        }
                    }
                    break;
                    case 0x07: // CDL T2 A
                        // case 0x08://CDL T2 B
                        {
                            DECLARE_ZERO_INIT_ARRAY(
                                uint8_t, cdl,
                                232 + SCSI_MODE_PAGE_MIN_HEADER_LENGTH); // need to include header length in this
                            bool sixByte = false;
                            // uint16_t headerLength = UINT16_C(0);
                            if (SUCCESS == get_SCSI_Mode_Page(
                                               device, MPC_CURRENT_VALUES, pageCode, subPageCode, M_NULLPTR, M_NULLPTR,
                                               true, cdl, 232 + SCSI_MODE_PAGE_MIN_HEADER_LENGTH, M_NULLPTR, &sixByte))
                            {
                                add_Feature_To_Supported_List(driveInfo->featuresSupported,
                                                              &driveInfo->numberOfFeaturesSupported,
                                                              "Command Duration Limits T2");
                               ...
                               // if we can read this page, then the device supports PATA Control
                    {
                        DECLARE_ZERO_INIT_ARRAY(
                            uint8_t, pataControl,
                            8 + SCSI_MODE_PAGE_MIN_HEADER_LENGTH); // need to include header length in this
                        bool pageRead = false;
                        bool sixByte  = false;
                        // uint16_t headerLength = UINT16_C(0);
                        uint16_t modeDataLen = UINT16_C(0);
                        if (SUCCESS == get_SCSI_Mode_Page(device, MPC_CURRENT_VALUES, pageCode, subPageCode, M_NULLPTR,
                                                          M_NULLPTR, true, pataControl,
                                                          8 + SCSI_MODE_PAGE_MIN_HEADER_LENGTH, M_NULLPTR, &sixByte))
                        {
                            uint16_t blockDescLen = UINT16_C(0);
                            uint8_t  devSpecific  = UINT8_C(0);
                            pageRead              = true;
                            get_SBC_Mode_Header_Blk_Desc_Fields(
                                sixByte, pataControl, 8 + SCSI_MODE_PAGE_MIN_HEADER_LENGTH, &modeDataLen, M_NULLPTR,
                                &devSpecific, M_NULLPTR, &blockDescLen, M_NULLPTR, M_NULLPTR);
                            if (devSpecific & BIT7)
                            {
                                driveInfo->isWriteProtected = true;
                        ...
                        {
                            add_Feature_To_Supported_List(driveInfo->featuresSupported,
                                                          &driveInfo->numberOfFeaturesSupported, "PATA Control");
                        }
                    }
                    break;
                    ...
                    {
                        DECLARE_ZERO_INIT_ARRAY(
                            uint8_t, ataFeatureControl,
                            16 + SCSI_MODE_PAGE_MIN_HEADER_LENGTH); // need to include header length in this
                        bool     pageRead     = false;
                        bool     sixByte      = false;
                        uint16_t headerLength = UINT16_C(0);
                        uint16_t modeDataLen  = UINT16_C(0);
                        if (SUCCESS == get_SCSI_Mode_Page(device, MPC_CURRENT_VALUES, pageCode, subPageCode, M_NULLPTR,
                                                          M_NULLPTR, true, ataFeatureControl,
                                                          16 + SCSI_MODE_PAGE_MIN_HEADER_LENGTH, M_NULLPTR, &sixByte))
                        {
                            uint16_t blockDescLen = UINT16_C(0);
                            uint8_t  devSpecific  = UINT8_C(0);
                            pageRead              = true;
                            get_SBC_Mode_Header_Blk_Desc_Fields(
                                sixByte, ataFeatureControl, 16 + SCSI_MODE_PAGE_MIN_HEADER_LENGTH, &modeDataLen,
                                M_NULLPTR, &devSpecific, M_NULLPTR, &blockDescLen, M_NULLPTR, M_NULLPTR);
                            if (devSpecific & BIT7)
                            {
                                driveInfo->isWriteProtected = true;
                        ...
                        if (pageRead && modeDataLen > 0)
                        {
                            if (get_bit_range_uint8(ataFeatureControl[headerLength + 4], 2, 0))
                            {
                                add_Feature_To_Supported_List(driveInfo->featuresSupported,
                                                              &driveInfo->numberOfFeaturesSupported,
                                                              "SATA Command Duration Limit [Enabled]");
                            }
                        }
                    }
                    break;
                ...
                case MP_PROTOCOL_SPECIFIC_PORT:
                    ...
                    case 0x00: // Protocol specific port (Use this to get whether SAS or FC or SCSI, etc)
                    {
                        ...
                        uint16_t modeDataLen  = UINT16_C(0);
                        if (SUCCESS == get_SCSI_Mode_Page(device, MPC_CURRENT_VALUES, pageCode, subPageCode, M_NULLPTR,
                                                          M_NULLPTR, true, protocolSpecificPort,
                                                          LEGACY_DRIVE_SEC_SIZE + SCSI_MODE_PAGE_MIN_HEADER_LENGTH,
                                                          M_NULLPTR, &sixByte))
                        {
                            uint16_t blockDescLen = UINT16_C(0);
                            uint8_t  devSpecific  = UINT8_C(0);
                            pageRead              = true;
                            get_SBC_Mode_Header_Blk_Desc_Fields(
                                sixByte, protocolSpecificPort, LEGACY_DRIVE_SEC_SIZE + SCSI_MODE_PAGE_MIN_HEADER_LENGTH,
                                &modeDataLen, M_NULLPTR, &devSpecific, M_NULLPTR, &blockDescLen, M_NULLPTR, M_NULLPTR);
                            if (devSpecific & BIT7)
                            {
                                driveInfo->isWriteProtected = true;
                            }
                    ...
                    case 0x01: // Phy control and discover mode page (SAS)
                    {
                        ...
                        if (pageRead && modeDataLen > 0)
                        {
                            ...
                            case 0x6:
                            {
                                ...
                                uint16_t phyPageLen = M_BytesTo2ByteValue(protocolSpecificPort[headerLength + 2],
                                                                          protocolSpecificPort[headerLength + 3]);
                                driveInfo->interfaceSpeedInfo.serialSpeed.numberOfPorts =
                                    protocolSpecificPort[headerLength + 7];
                                uint8_t phyCount = UINT8_C(0);
                                // now we need to go through the descriptors for each phy
                                for (; phyDescriptorIter <
                                           C_CAST(uint16_t,
                                                  M_Min(C_CAST(uint16_t, phyPageLen + headerLength),
                                                        C_CAST(uint16_t, LEGACY_DRIVE_SEC_SIZE + headerLength))) &&
                                       phyCount < C_CAST(uint8_t, MAX_PORTS);
                                     phyDescriptorIter += 48, phyCount++)
                                {
                    ...
                    case 0x03: // Negotiated Settings (Parallel SCSI)
                    {
                        ...
                        if (pageRead && modeDataLen > 0)
                        {
                            ...
                            case 0x1: // parallel scsi
                            {
                                ...
                                uint16_t scalingMultiplier     = UINT16_C(0);
                ...
                case MP_POWER_CONDTION:
                    ...
                    case 0x00: // EPC
                    {
                        ...
                        uint32_t epcFeatureStringLength = UINT32_C(0);
                        // read the default values to check if it's supported...then try the current page...
                        bool defaultsRead = false;
                        bool sixByte      = false;
                        DECLARE_ZERO_INIT_ARRAY(
                            uint8_t, powerConditions,
                            40 + SCSI_MODE_PAGE_MIN_HEADER_LENGTH); // need to include header length in this
                        bool     pageRead     = false;
                        uint16_t headerLength = UINT16_C(0);
                        uint16_t modeDataLen  = UINT16_C(0);
                        if (SUCCESS == get_SCSI_Mode_Page(device, MPC_DEFAULT_VALUES, pageCode, subPageCode, M_NULLPTR,
                                                          M_NULLPTR, true, powerConditions,
                                                          40 + SCSI_MODE_PAGE_MIN_HEADER_LENGTH, M_NULLPTR, &sixByte))
                        {
                            uint16_t blockDescLen = UINT16_C(0);
                            uint8_t  devSpecific  = UINT8_C(0);
                            defaultsRead          = true;
                            ...
                            get_SBC_Mode_Header_Blk_Desc_Fields(
                                sixByte, powerConditions, 40 + SCSI_MODE_PAGE_MIN_HEADER_LENGTH, &modeDataLen,
                                M_NULLPTR, &devSpecific, M_NULLPTR, &blockDescLen, M_NULLPTR, M_NULLPTR);
                            if (devSpecific & BIT7)
                            {
                                driveInfo->isWriteProtected = true;
                        ...
                        if (defaultsRead && modeDataLen > 0)
                        {
                            ...
                            {
                                if (epcFeatureString == M_NULLPTR)
                                {
                                    epcFeatureStringLength = 4;
                                    epcFeatureString =
                                        M_REINTERPRET_CAST(char*, safe_calloc(epcFeatureStringLength, sizeof(char)));
                                }
                                else
                                {
                                    epcFeatureStringLength = 4;
                                    char* temp =
                                        C_CAST(char*, safe_reallocf(M_REINTERPRET_CAST(void**, &epcFeatureString),
                                                                    epcFeatureStringLength * sizeof(char)));
                                    if (temp != M_NULLPTR)
                                    {
                        ...
                        {
                            if (powerConditions[1 + headerLength] > 0x0A &&
                                (powerConditions[2 + headerLength] & BIT0 || powerConditions[3 + headerLength] & BIT0 ||
                                 powerConditions[3 + headerLength] & BIT1 || powerConditions[3 + headerLength] & BIT2 ||
                    ...
                    case 0xF1: // ata power conditions
                    {
                        ...
                        uint16_t modeDataLen  = UINT16_C(0);
                        if (SUCCESS == get_SCSI_Mode_Page(device, MPC_CURRENT_VALUES, pageCode, subPageCode, M_NULLPTR,
                                                          M_NULLPTR, true, ataPowerConditions,
                                                          16 + SCSI_MODE_PAGE_MIN_HEADER_LENGTH, M_NULLPTR, &sixByte))
                        {
                            uint16_t blockDescLen = UINT16_C(0);
                            uint8_t  devSpecific  = UINT8_C(0);
                            pageRead              = true;
                            get_SBC_Mode_Header_Blk_Desc_Fields(
                                sixByte, ataPowerConditions, 16 + SCSI_MODE_PAGE_MIN_HEADER_LENGTH, &modeDataLen,
                                M_NULLPTR, &devSpecific, M_NULLPTR, &blockDescLen, M_NULLPTR, M_NULLPTR);
                            if (devSpecific & BIT7)
                            {
                                driveInfo->isWriteProtected = true;
                        ...
                        if (pageRead && modeDataLen > 0)
                        {
                            ...
                            {
                                add_Feature_To_Supported_List(driveInfo->featuresSupported,
                                                              &driveInfo->numberOfFeaturesSupported, "APM [Enabled]");
                            }
                            else
                            {
                                add_Feature_To_Supported_List(driveInfo->featuresSupported,
                                                              &driveInfo->numberOfFeaturesSupported, "APM");
                            }
                        }
                    }
                    break;
                ...
                case MP_INFORMATION_EXCEPTIONS_CONTROL:
                    ...
                    {
                        DECLARE_ZERO_INIT_ARRAY(
                            uint8_t, informationalExceptions,
                            12 + SCSI_MODE_PAGE_MIN_HEADER_LENGTH); // need to include header length in this
                        bool     pageRead     = false;
                        bool     sixByte      = false;
                        uint16_t headerLength = UINT16_C(0);
                        uint16_t modeDataLen  = UINT16_C(0);
                        if (SUCCESS == get_SCSI_Mode_Page(device, MPC_CURRENT_VALUES, pageCode, subPageCode, M_NULLPTR,
                                                          M_NULLPTR, true, informationalExceptions,
                                                          12 + SCSI_MODE_PAGE_MIN_HEADER_LENGTH, M_NULLPTR, &sixByte))
                        {
                            uint16_t blockDescLen = UINT16_C(0);
                            uint8_t  devSpecific  = UINT8_C(0);
                            pageRead              = true;
                            get_SBC_Mode_Header_Blk_Desc_Fields(
                                sixByte, informationalExceptions, 12 + SCSI_MODE_PAGE_MIN_HEADER_LENGTH, &modeDataLen,
                                M_NULLPTR, &devSpecific, M_NULLPTR, &blockDescLen, M_NULLPTR, M_NULLPTR);
                            if (devSpecific & BIT7)
                            {
                                driveInfo->isWriteProtected = true;
                    ...
                    case 0x01: // Background control
                    {
                        ...
                        uint32_t bmsStringLength   = UINT32_C(0);
                        uint32_t bmsPSStringLength = UINT32_C(0);
                        DECLARE_ZERO_INIT_ARRAY(
                            uint8_t, backgroundControl,
                            16 + SCSI_MODE_PAGE_MIN_HEADER_LENGTH); // need to include header length in this
                        bool     pageRead     = false;
                        uint16_t modeDataLen  = UINT16_C(0);
                        bool     defaultsRead = false;
                        ...
                        uint16_t headerLength = UINT16_C(0);
                        if (SUCCESS == get_SCSI_Mode_Page(device, MPC_DEFAULT_VALUES, pageCode, subPageCode, M_NULLPTR,
                                                          M_NULLPTR, true, backgroundControl,
                                                          16 + SCSI_MODE_PAGE_MIN_HEADER_LENGTH, M_NULLPTR, &sixByte))
                        {
                            uint16_t blockDescLen = UINT16_C(0);
                            uint8_t  devSpecific  = UINT8_C(0);
                            ...
                            get_SBC_Mode_Header_Blk_Desc_Fields(
                                sixByte, backgroundControl, 16 + SCSI_MODE_PAGE_MIN_HEADER_LENGTH, &modeDataLen,
                                M_NULLPTR, &devSpecific, M_NULLPTR, &blockDescLen, M_NULLPTR, M_NULLPTR);
                            if (devSpecific & BIT7)
                            {
                                driveInfo->isWriteProtected = true;
                        ...
                        if (defaultsRead && modeDataLen > 0)
                        {
                            ...
                            {
                                if (bmsString == M_NULLPTR)
                                {
                                    bmsStringLength = 50;
                                    bmsString = M_REINTERPRET_CAST(char*, safe_calloc(bmsStringLength, sizeof(char)));
                                }
                                else
                                {
                                    bmsStringLength = 50;
                                    char* temp =
                                        M_REINTERPRET_CAST(char*, safe_reallocf(M_REINTERPRET_CAST(void**, &bmsString),
                                                                                bmsStringLength * sizeof(char)));
                                    if (temp != M_NULLPTR)
                                    {
                                        bmsString = temp;
                            ...
                            {
                                if (bmsPSString == M_NULLPTR)
                                {
                                    bmsPSStringLength = 50;
                                    bmsPSString =
                                        M_REINTERPRET_CAST(char*, safe_calloc(bmsPSStringLength, sizeof(char)));
                                }
                                else
                                {
                                    bmsPSStringLength = 50;
    ...
    if (device && driveInfo && scsiInfo)
    {
            ...
            device->drive_info.interface_type != MMC_INTERFACE && device->drive_info.interface_type != SD_INTERFACE)
        {
            ...
            if (supportedDiagnostics != M_NULLPTR)
            {
                ...
                {
                    // try this request first. If it fails, then most likely we need to use the SCSI 2 method below
                    // instead
                    if (SUCCESS == scsi_Receive_Diagnostic_Results(device, true, DIAG_PAGE_SUPPORTED_PAGES,
                                                                   supportedDiagsLength, supportedDiagnostics, 15))
                    {
                        if (supportedDiagnostics[0] == DIAG_PAGE_SUPPORTED_PAGES &&
                            supportedDiagnostics[1] == 0) // validate returned page data
                        {
                            gotDiagData = true;
                        }
                    }
                ...
                if (!gotDiagData)
                {
                    ...
                    {
                        if (SUCCESS == scsi_Receive_Diagnostic_Results(device, false, 0, supportedDiagsLength,
                                                                       supportedDiagnostics, 15))
                        {
                            gotDiagData = true;
                        }
                    }
                ...
                if (gotDiagData)
                {
                    // confirm the page code in case old devices did not respond correctly due to not supporting the
                    // page format bit. confirm the list of supported pages by checking the page code and byte 1 are
                    // both set to zero
                    if (supportedDiagnostics[0] == DIAG_PAGE_SUPPORTED_PAGES && supportedDiagnostics[1] == 0)
                    {
                        ...
                        for (uint32_t iter = UINT16_C(4);
                             iter < C_CAST(uint32_t, pageLength + UINT16_C(4)) && iter < supportedDiagsLength; ++iter)
                        {
                            ...
                            case DIAG_PAGE_REBUILD_ASSIST:
                                add_Feature_To_Supported_List(driveInfo->featuresSupported,
                                                              &driveInfo->numberOfFeaturesSupported, "Rebuild Assist");
                                break;
                            case 0x90:
                                if (is_Seagate_Family(device) == SEAGATE)
                                {
...
eReturnValues get_SCSI_Drive_Information(const tDevice* device, ptrDriveInformationSAS_SATA driveInfo)
{
        ...
        (device->drive_info.drive_type != ATA_DRIVE && device->drive_info.drive_type != NVME_DRIVE))
    {
        ...
        // TODO: Find a better way to remove the large check above which may not work out well in some cases, but should
        // reduce false detection on USB among other interfaces
        ...
        (device->drive_info.passThroughHacks.scsiHacks.securityProtocolSupported ||
         SUCCESS == scsi_SecurityProtocol_In(device, SECURITY_PROTOCOL_INFORMATION, 0, false, 0,
                                             M_NULLPTR))) // security protocol commands introduced in SPC4. TODO: may
                                                          // need to drop to SPC3 for some devices. Need to investigate
    {
        ...
        {
            if (SUCCESS ==
                scsi_SecurityProtocol_In(device, SECURITY_PROTOCOL_INFORMATION, 0, false, 512, securityProtocols))
            {
                if (SUCCESS == get_Security_Features_From_Security_Protocol(device, &driveInfo->securityInfo,
                                                                            securityProtocols, 512))
                {
                    if (driveInfo->securityInfo.tcg)
                    {
                        driveInfo->encryptionSupport = ENCRYPTION_SELF_ENCRYPTING;
                    ...
                    {
                        add_Feature_To_Supported_List(driveInfo->featuresSupported,
                                                      &driveInfo->numberOfFeaturesSupported, "CbCS");
                    }
                    if (driveInfo->securityInfo.tapeEncryption)
                    {
                        add_Feature_To_Supported_List(driveInfo->featuresSupported,
                                                      &driveInfo->numberOfFeaturesSupported, "Tape Encryption");
                    }
                    ...
                    {
                        add_Feature_To_Supported_List(driveInfo->featuresSupported,
                                                      &driveInfo->numberOfFeaturesSupported, "IKE V2 SCSI");
                    }
                    if (driveInfo->securityInfo.sdAssociation)
                    {
                        add_Feature_To_Supported_List(driveInfo->featuresSupported,
                                                      &driveInfo->numberOfFeaturesSupported, "SD Association");
                    }
                    if (driveInfo->securityInfo.dmtfSecurity)
                    {
                        add_Feature_To_Supported_List(driveInfo->featuresSupported,
                                                      &driveInfo->numberOfFeaturesSupported, "DMTF Security");
                    }
                    if (driveInfo->securityInfo.nvmeReserved)
                    {
                        add_Feature_To_Supported_List(driveInfo->featuresSupported,
                                                      &driveInfo->numberOfFeaturesSupported, "NVMe Reserved");
                    }
                    if (driveInfo->securityInfo.nvme)
                    {
                        add_Feature_To_Supported_List(driveInfo->featuresSupported,
                                                      &driveInfo->numberOfFeaturesSupported, "NVMe RPMB");
                    }
                    if (driveInfo->securityInfo.scsa)
                    {
                        add_Feature_To_Supported_List(driveInfo->featuresSupported,
                    ...
                    {
                        add_Feature_To_Supported_List(driveInfo->featuresSupported,
                                                      &driveInfo->numberOfFeaturesSupported, "JEDEC UFS");
                    }
                    if (driveInfo->securityInfo.sdTrustedFlash)
                    {
                        add_Feature_To_Supported_List(driveInfo->featuresSupported,
                                                      &driveInfo->numberOfFeaturesSupported, "SD Trusted Flash");
                    }
                    if (driveInfo->securityInfo.ieee1667)
                    {
                        add_Feature_To_Supported_List(driveInfo->featuresSupported,
                                                       ...
                                                       uint32_t                identifyDataLength)
{
    ...
    driveInfo->controllerData.ieeeOUI =
        M_BytesTo4ByteValue(0, nvmeIdentifyData[75], nvmeIdentifyData[74], nvmeIdentifyData[73]);
    // controller ID
    ...
    driveInfo->controllerData.warningCompositeTemperatureThreshold =
        M_BytesTo2ByteValue(nvmeIdentifyData[267], nvmeIdentifyData[266]);
    // critical composite temperature
    driveInfo->controllerData.criticalCompositeTemperatureThreshold =
        M_BytesTo2ByteValue(nvmeIdentifyData[269], nvmeIdentifyData[268]);
    // total nvm capacity
    for (uint8_t i = UINT8_C(0); i < 16; ++i)
    {
        driveInfo->controllerData.totalNVMCapacity[i] = nvmeIdentifyData[295 + i];
    }
    driveInfo->controllerData.totalNVMCapacityD =
        convert_128bit_to_double(&driveInfo->controllerData.totalNVMCapacity[0]);
    // unallocated nvm capacity
    for (uint8_t i = UINT8_C(0); i < 16; ++i)
    {
        driveInfo->controllerData.unallocatedNVMCapacity[i] = nvmeIdentifyData[296 + i];
    }
    driveInfo->controllerData.unallocatedNVMCapacityD =
        convert_128bit_to_double(&driveInfo->controllerData.unallocatedNVMCapacity[0]);
    // DST info
    if (nvmeIdentifyData[256] & BIT4) // DST command is supported
    {
        // set Long DST Time before reading the log
        driveInfo->controllerData.longDSTTimeMinutes =
            M_BytesTo2ByteValue(nvmeIdentifyData[317], nvmeIdentifyData[316]);
        ...
        if (SUCCESS == nvme_Get_Log_Page(device, &dstLogOpts))
        {
            ...
            uint32_t latestDSTOffset = UINT32_C(4);
            ...
            if (status != 0x0F) // a status of F means this is an unused entry
            {
                ...
                if (nvmeDSTLog[latestDSTOffset + 2] & BIT1)
                {
                    driveInfo->dstInfo.errorLBA =
                        M_BytesTo8ByteValue(nvmeDSTLog[latestDSTOffset + 23], nvmeDSTLog[latestDSTOffset + 22],
                                            nvmeDSTLog[latestDSTOffset + 12], nvmeDSTLog[latestDSTOffset + 20],
                                            nvmeDSTLog[latestDSTOffset + 19], nvmeDSTLog[latestDSTOffset + 18],
                                            nvmeDSTLog[latestDSTOffset + 17], nvmeDSTLog[latestDSTOffset + 16]);
                }
                else
                {
                    driveInfo->dstInfo.errorLBA = UINT64_MAX;
    ...
    driveInfo->controllerData.maxNumberOfNamespaces =
        M_BytesTo4ByteValue(nvmeIdentifyData[519], nvmeIdentifyData[518], nvmeIdentifyData[517], nvmeIdentifyData[516]);
    ...
    if (nvmeIdentifyData[256] & BIT0)
    {
        ...
        if (SUCCESS ==
            nvme_Security_Receive(device, SECURITY_PROTOCOL_INFORMATION, 0, 0, supportedSecurityProtocols, 512))
        {
            if (SUCCESS == get_Security_Features_From_Security_Protocol(device, &driveInfo->securityInfo,
                                                                        supportedSecurityProtocols, 512))
            {
                if (driveInfo->securityInfo.tcg)
                {
                    ...
                    add_Feature_To_Supported_List(driveInfo->controllerData.controllerFeaturesSupported,
                                                  &driveInfo->controllerData.numberOfControllerFeatures, "TCG");
                }
                if (driveInfo->securityInfo.cbcs)
                {
                    add_Feature_To_Supported_List(driveInfo->controllerData.controllerFeaturesSupported,
                                                  &driveInfo->controllerData.numberOfControllerFeatures, "CbCS");
                }
                if (driveInfo->securityInfo.tapeEncryption)
                {
                    add_Feature_To_Supported_List(driveInfo->controllerData.controllerFeaturesSupported,
                ...
                {
                    add_Feature_To_Supported_List(driveInfo->controllerData.controllerFeaturesSupported,
                                                  &driveInfo->controllerData.numberOfControllerFeatures, "IKE V2 SCSI");
                }
                if (driveInfo->securityInfo.sdAssociation)
                {
                    add_Feature_To_Supported_List(driveInfo->controllerData.controllerFeaturesSupported,
                ...
                {
                    add_Feature_To_Supported_List(driveInfo->controllerData.controllerFeaturesSupported,
                                                  &driveInfo->controllerData.numberOfControllerFeatures, "NVMe RPMB");
                }
                if (driveInfo->securityInfo.scsa)
                {
                    add_Feature_To_Supported_List(driveInfo->controllerData.controllerFeaturesSupported,
                                                  &driveInfo->controllerData.numberOfControllerFeatures, "SCSA");
                }
                if (driveInfo->securityInfo.jedecUFS)
                {
                    add_Feature_To_Supported_List(driveInfo->controllerData.controllerFeaturesSupported,
                                                  &driveInfo->controllerData.numberOfControllerFeatures, "JEDEC UFS");
                }
                if (driveInfo->securityInfo.sdTrustedFlash)
                {
                    add_Feature_To_Supported_List(driveInfo->controllerData.controllerFeaturesSupported,
                ...
                {
                    add_Feature_To_Supported_List(driveInfo->controllerData.controllerFeaturesSupported,
                                                  &driveInfo->controllerData.numberOfControllerFeatures, "IEEE 1667");
                }
                if (driveInfo->securityInfo.ataDeviceServer)
                {
                    add_Feature_To_Supported_List(driveInfo->controllerData.controllerFeaturesSupported,
        ...
        {
            // NOTE: For whatever reason the security commands did not complete despite the drive supporting them to
            // read the list of supported protocols set the "blocked commaands" flag
    ...
    {
        add_Feature_To_Supported_List(driveInfo->controllerData.controllerFeaturesSupported,
                                      &driveInfo->controllerData.numberOfControllerFeatures, "Format NVM");
    }
    if (nvmeIdentifyData[256] & BIT2)
    {
        add_Feature_To_Supported_List(driveInfo->controllerData.controllerFeaturesSupported,
                                      &driveInfo->controllerData.numberOfControllerFeatures, "Firmware Update");
    }
    if (nvmeIdentifyData[256] & BIT3)
    {
        add_Feature_To_Supported_List(driveInfo->controllerData.controllerFeaturesSupported,
                                      &driveInfo->controllerData.numberOfControllerFeatures, "Namespace Management");
    }
    if (nvmeIdentifyData[256] & BIT4)
    {
        add_Feature_To_Supported_List(driveInfo->controllerData.controllerFeaturesSupported,
                                      &driveInfo->controllerData.numberOfControllerFeatures, "Device Self Test");
    }
    ...
    {
        add_Feature_To_Supported_List(driveInfo->controllerData.controllerFeaturesSupported,
                                      &driveInfo->controllerData.numberOfControllerFeatures, "Doorbell Buffer Config");
    }

    // Before we memset the identify data, add some namespace features
    if (nvmeIdentifyData[520] & BIT1)
    {
        add_Feature_To_Supported_List(driveInfo->namespaceData.namespaceFeaturesSupported,
                                      &driveInfo->namespaceData.numberOfNamespaceFeatures, "Write Uncorrectable");
    }
    if (nvmeIdentifyData[520] & BIT2)
    {
        add_Feature_To_Supported_List(driveInfo->namespaceData.namespaceFeaturesSupported,
                                      &driveInfo->namespaceData.numberOfNamespaceFeatures, "Dataset Management");
    }
    if (nvmeIdentifyData[520] & BIT3)
    {
        add_Feature_To_Supported_List(driveInfo->namespaceData.namespaceFeaturesSupported,
                                      &driveInfo->namespaceData.numberOfNamespaceFeatures, "Write Zeros");
    }
    if (nvmeIdentifyData[520] & BIT5)
    {
        add_Feature_To_Supported_List(driveInfo->namespaceData.namespaceFeaturesSupported,
                                      &driveInfo->namespaceData.numberOfNamespaceFeatures, "Persistent Reservations");
    }
    return ret;
}
                                                      ...
                                                      uint32_t                identifyDataLength)
{
    ...
    driveInfo->namespaceData.namespaceSize =
        M_BytesTo8ByteValue(nvmeIdentifyData[7], nvmeIdentifyData[6], nvmeIdentifyData[5], nvmeIdentifyData[4],
                            nvmeIdentifyData[3], nvmeIdentifyData[2], nvmeIdentifyData[1], nvmeIdentifyData[0]) -
        1; // spec says this is 0 to (n-1)!
    driveInfo->namespaceData.namespaceCapacity =
        M_BytesTo8ByteValue(nvmeIdentifyData[15], nvmeIdentifyData[14], nvmeIdentifyData[13], nvmeIdentifyData[12],
                            nvmeIdentifyData[11], nvmeIdentifyData[10], nvmeIdentifyData[9], nvmeIdentifyData[8]);
    driveInfo->namespaceData.namespaceUtilization =
        M_BytesTo8ByteValue(nvmeIdentifyData[23], nvmeIdentifyData[22], nvmeIdentifyData[21], nvmeIdentifyData[20],
                            nvmeIdentifyData[19], nvmeIdentifyData[18], nvmeIdentifyData[17], nvmeIdentifyData[16]);
    ...
    if (nvmeIdentifyData[28] & BIT0)
    {
        ...
        {
            add_Feature_To_Supported_List(driveInfo->namespaceData.namespaceFeaturesSupported,
                                          &driveInfo->namespaceData.numberOfNamespaceFeatures, "Protection Type 1");
        }
    }
    if (nvmeIdentifyData[28] & BIT1)
    {
        ...
        {
            add_Feature_To_Supported_List(driveInfo->namespaceData.namespaceFeaturesSupported,
                                          &driveInfo->namespaceData.numberOfNamespaceFeatures, "Protection Type 2");
        }
    }
    if (nvmeIdentifyData[28] & BIT2)
    {
        ...
        {
            add_Feature_To_Supported_List(driveInfo->namespaceData.namespaceFeaturesSupported,
                                          &driveInfo->namespaceData.numberOfNamespaceFeatures, "Protection Type 3");
        }
    }
    if (nvmeIdentifyData[30] & BIT0)
    {
        add_Feature_To_Supported_List(driveInfo->namespaceData.namespaceFeaturesSupported,
                                      &driveInfo->namespaceData.numberOfNamespaceFeatures, "Namespace Sharing");
    }
    return ret;
}
...
void print_NVMe_Device_Information(ptrDriveInformationNVMe driveInfo)
{
    ...
    {
        printf("%" PRIu16 ".%" PRIu8 ".%" PRIu8 "\n", driveInfo->controllerData.majorVersion,
               driveInfo->controllerData.minorVersion, driveInfo->controllerData.tertiaryVersion);
    }
    else
    {
        print_str("Not reported (NVMe 1.1 or older)\n");
    }
    ...
    if (driveInfo->controllerData.totalNVMCapacityD > 0)
    {
        ...
        printf("\tTotal NVM Capacity (%s/%s): %0.02f/%0.02f\n", mTotalCapUnit, totalCapUnit, mTotalCapacity,
               totalCapacity);
    ...
    if (driveInfo->smartData.valid)
    {
        ...
        print_str("\tLast DST information:\n");
        if (driveInfo->dstInfo.informationValid)
        {
                ...
                driveInfo->smartData.powerOnHoursD)
            {
                ...
                print_str("\t\tTime since last DST (hours): ");
            ...
            {
                print_str("\t\tDST has never been run\n");
            }
        }
        else
        {
        ...
        // Long DST time
        print_str("\tLong Drive Self Test Time: ");
        ...
        print_str("\tAnnualized Workload Rate (TB/yr): ");
...
#    define MINUTES_IN_1_YEAR 525600.0
    ...
    case ENCRYPTION_FULL_DISK:
        print_str("Full Disk Encryption\n");
        break;
    ...
    if (driveInfo->namespaceData.valid)
    {
        ...
        printf("\tNamespace Utilization (%s/%s): %0.02f/%0.02f\n", mUtilizationUnit, utilizationUnit, nvmMUtilization,
               nvmUtilization);
        ...
        print_str("\tLogical Block Size Relative Performance: ");
        switch (driveInfo->namespaceData.relativeFormatPerformance)
        {
        case 0:
            print_str("Best Performance\n");
            break;
        case 1:
            print_str("Better Performance\n");
            break;
        case 2:
            print_str("Good Performance\n");
            break;
        case 3:
            print_str("Degraded Performance\n");
            break;
        default: // this case shouldn't ever happen...just reducing a warning - TJE
            print_str("Unknown Performance\n");
            break;
        }
    ...
    {
        print_str("\tERROR: Could not get namespace data!\n");
    }
    print_str("\n");
}
...
void print_SAS_Sata_Device_Information(ptrDriveInformationSAS_SATA driveInfo)
{
    ...
    {
        print_str("\t\tCurrent Temperature (C): Not Reported\n");
    }
    // Highest Temperature
    if (driveInfo->temperatureData.highestValid)
    {
    ...
    {
        print_str("\t\tHighest Temperature (C): Not Reported\n");
    }
    // Lowest Temperature
    if (driveInfo->temperatureData.lowestValid)
    {
    ...
    {
        print_str("\t\tLowest Temperature (C): Not Reported\n");
    }
    if (driveInfo->humidityData.humidityDataValid)
    {
        // Humidity Data
        print_str("\tHumidity Data:\n");
        if (driveInfo->humidityData.humidityDataValid)
        {
    ...
    {
        // Logical Sector Size
        print_str("\tLogical Sector Size (B): Format Corrupt\n");
        // Physical Sector Size
        print_str("\tPhysical Sector Size (B): Format Corrupt\n");
        // Sector Alignment
        print_str("\tSector Alignment: Format Corrupt\n");
    }
    else
    {
        // Logical Sector Size
    ...
    if (driveInfo->interfaceSpeedInfo.speedIsValid)
    {
        if (driveInfo->interfaceSpeedInfo.speedType == INTERFACE_SPEED_SERIAL)
        {
            if (driveInfo->interfaceSpeedInfo.serialSpeed.numberOfPorts > 0)
            {
                if (driveInfo->interfaceSpeedInfo.serialSpeed.numberOfPorts == 1)
                {
                    ...
                    print_str("\n");
                    print_str("\t\tNegotiated Speed (Gb/s): ");
        ...
        else if (driveInfo->interfaceSpeedInfo.speedType == INTERFACE_SPEED_PARALLEL)
        {
            ...
            print_str("\n");
            print_str("\t\tNegotiated Speed (MB/s): ");
            if (driveInfo->interfaceSpeedInfo.parallelSpeed.negotiatedValid)
            {
...
#    define MINUTES_IN_1_YEAR 525600.0
    ...
    {
        print_str("\tCache Size (MiB): Not Reported\n");
    }
    ...
    if (driveInfo->readLookAheadSupported)
    {
        if (driveInfo->readLookAheadEnabled)
        {
            print_str("\tRead Look-Ahead: Enabled\n");
        }
        else
        {
            print_str("\tRead Look-Ahead: Disabled\n");
        }
    }
    else
    {
        print_str("\tRead Look-Ahead: Not Supported\n");
    }
    // NVCache (!NV_DIS bit from caching MP)
    if (driveInfo->nvCacheSupported)
    {
        print_str("\tNon-Volatile Cache: ");
    ...
    if (driveInfo->writeCacheSupported)
    {
        if (driveInfo->writeCacheEnabled)
        {
            print_str("\tWrite Cache: Enabled\n");
        }
        else
        {
            print_str("\tWrite Cache: Disabled\n");
        }
    }
    else
    {
        print_str("\tWrite Cache: Not Supported\n");
    }
    if (driveInfo->lowCurrentSpinupValid)
    {
        ...
        else
        {
            ...
            {
                print_str("\tLow Current Spinup: Enabled\n");
            }
            else
            {
                print_str("\tLow Current Spinup: Disabled\n");
            }
        }
    }
    // SMART Status
    ...
    case 1: // bad
        ...
        break;
    default: // unknown
        print_str("Unknown or Not Supported\n");
        break;
    }
    // ATA Security Infomation
    print_str("\tATA Security Information: ");
    if (driveInfo->ataSecurityInformation.securitySupported)
    {
        ...
        {
            print_str(", Password Count Expired");
        }
        print_str("\n");
    }
    else
    ...
    if (driveInfo->zonedDevice > 0)
    {
        ...
        case 0x3: // reserved
            ...
            break;
        default:
            print_str("Not a Zoned Device\n");
            break;
        }
    }
    print_str("\tFirmware Download Support: ");
    if (driveInfo->fwdlSupport.downloadSupported)
    {
        print_str("Full"); // changed to "Full" from "Immediate" since this makes more sense...-TJE
        if (driveInfo->fwdlSupport.segmentedSupported)
        {
            print_str(", Segmented");
            if (driveInfo->fwdlSupport.seagateDeferredPowerCycleRequired)
            {
                print_str(" as Deferred - Power Cycle Activation Only");
            }
        }
    ...
    print_str("\tSpecifications Supported:\n");
    ...
    {
        print_str("\t\tNone reported by device.\n");
    }
    // Features Supported
    print_str("\tFeatures Supported:\n");
    ...
    {
        print_str("\t\tNone reported or an error occurred while trying to determine\n\t\tthe features.\n");
    }
    // Adapter information
    print_str("\tAdapter Information:\n");
    print_str("\t\tAdapter Type: ");
    ...
    {
        print_str("\t\tSpecifier ID: ");
        printf("%04" PRIX32 "h\n", driveInfo->adapterInformation.specifierID);
    }
    if (driveInfo->lunCount > 1)
    {
        print_str("This device has multiple actuators. Some commands/features may affect more than one actuator.\n");
    }
}
                                         ...
                                         ptrDriveInformationSAS_SATA ataDriveInfo)
{
    ...
    if (externalDriveInfo != M_NULLPTR && scsiDriveInfo != M_NULLPTR && ataDriveInfo != M_NULLPTR)
    {
        ...
        uint16_t scsiSpecNumber = UINT16_C(0);
                                              ...
                                              ptrDriveInformationNVMe     nvmeDriveInfo)
{
    ...
    if (externalDriveInfo != M_NULLPTR && scsiDriveInfo != M_NULLPTR && nvmeDriveInfo != M_NULLPTR)
    {
        ...
        if (nvmeDriveInfo->smartData.valid)
        {
            ...
            externalDriveInfo->temperatureData.currentTemperature =
                C_CAST(int16_t, nvmeDriveInfo->smartData.compositeTemperatureKelvin) - INT16_C(273);
            externalDriveInfo->temperatureData.temperatureDataValid = true;
            // Workload (reads, writes)
            externalDriveInfo->totalBytesRead =
                C_CAST(uint64_t, nvmeDriveInfo->smartData.dataUnitsReadD * 512 *
                                     1000); // this is a count of 512B units, so converting to bytes
            externalDriveInfo->totalLBAsRead = C_CAST(uint64_t, nvmeDriveInfo->smartData.dataUnitsReadD * 512 * 1000 /
                                                                    nvmeDriveInfo->namespaceData.formattedLBASizeBytes);
            externalDriveInfo->totalBytesWritten =
                C_CAST(uint64_t, nvmeDriveInfo->smartData.dataUnitsWrittenD * 512 *
                                     1000); // this is a count of 512B units, so converting to bytes
            externalDriveInfo->totalLBAsWritten =
                C_CAST(uint64_t, nvmeDriveInfo->smartData.dataUnitsWrittenD * 512 * 1000 /
                                     nvmeDriveInfo->namespaceData.formattedLBASizeBytes);
        ...
        uint16_t nvmeFeatNumber = UINT16_C(0);
        // controller features
        for (;
        ...
        if (nvmeDriveInfo->namespaceData.valid)
        {
            // namespace features
            nvmeFeatNumber = 0;
            for (; extFeatNumber < MAX_FEATURES &&
        ...
        else
        {
                     ...
                     device->drive_info.drive_type == NVME_DRIVE && *nvmeDriveInfo && *scsiDriveInfo)
            {
                *usbDriveInfo = M_REINTERPRET_CAST(ptrDriveInformation, safe_calloc(1, sizeof(driveInformation)));
                ...
                {
                    ret = MEMORY_FAILURE;
                    print_str("Error allocating memory for USB - NVMe drive info\n");
                }
            }
            else
            {
                ...
                {
                    ret = MEMORY_FAILURE;
                    print_str("Error allocating memory to get device information.\n");
                }
            }
        }
    }
