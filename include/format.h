...
extern "C"
{
    ...
    //!   \param[out] fastFormatSupported = bool that will hold if fast format is supported. May be M_NULLPTR if you
    //!   don't care about checking for fast format support.
    //!
    //  Exit:
    ...
    M_PARAM_RO(1)
    M_PARAM_WO(2)
    OPENSEA_OPERATIONS_API bool is_Format_Unit_Supported(const tDevice* device, bool* fastFormatSupported);
    ...
    //!
    //  Exit:
    //!   \return SUCCESS = format unit not in progress, IN_PROGRESS = format unit in progress, !SUCCESS = something
    ...
    M_PARAM_RO(1)
    M_PARAM_WO(2)
    OPENSEA_OPERATIONS_API eReturnValues get_Format_Progress(const tDevice* device, double* percentComplete);
    ...
    typedef enum eFormatPatternEnum
    {
    ...
    typedef struct s_runFormatUnitParameters
    {
        ...
        bool securityInitialize; // Not supported on Seagate products. Recommended to use sanitize instead. This ignores
                                 ...
                                 // reallocated sectors
        bool stopOnListError; // Only used if cmplst is zero and dpry is zero. If the previous condition is met and this
                              // is true, the device will stop the format if it cannot access a list, otherwise it will
                              // continue processing the command. If unsure, leave false
        bool disableImmediate; // Only set this is you want to wait for the device to completely format itself before
                               // returning status! You cannot poll while this is happening. It is recommended that this
                               // is left false!
        bool changeProtectionType; // When this is set to true, the protection type below will be used to set parameters
                                   // going to the drive, otherwise the current settings in the device structure will be
                                   // used.
        uint8_t protectionType; // if unsure, use 0. This will set the proper bit combinations for each protection type
        uint8_t protectionIntervalExponent; // Only used on protection types 2 or 3. Ignored and unused otherwise since
                                            // other types require this to be zero. If unsure, leave as zero
    ...
    //
    //! \brief   Description:  runs or starts a format unit operation on a specified device. All formats through this
    //! function are started with immed bit set to 1.
    //
    //  Entry:
    //!   \param[in] device = file descriptor
    //!   \param[in] formatType = use this to specify a standard format or fast format
    //!   \param[in] currentBlockSize = use the current logical block size of the device for format (next parameter
    //!   ignored) \param[in] newBlockSize = if currentBlockSize is false, this is the new logical block size to format
    //!   the drive with. \param[in] gList = pointer to the glist to use during format. When M_NULLPTR, the device will
    //!   use the current glist unless the completeList bool is set to true. \param[in] glistSize = size of the glist
    //!   pointed to by gList \param[in] completeList = set to true to say the provided glist is the complete glist for
    //!   the device. If this is true and gList is M_NULLPTR, then this will clear the glist. \param[in]
    //!   disablePrimaryList = set to true to disable using the primary list during a format. \param[in]
    //!   disableCertification = set to true to disable certification \param[in] pattern = pointer to a pattern to use
    //!   during the format. If M_NULLPTR, the device's default patter is used. \param[in] patternLength = length of the
    //!   data pointed to by pattern \param[in] securityInitialize = set to true to set the security initialize bit
    //!   which requests previously reallocated areas to be overwritten. (Seagate drive's don't currently support this)
    //!   SBC spec recommends using Sanitize instead of this bit to overwrite previously reallocated sectors. \param[in]
    //!   pollForProgress = set to true for this function to poll for progress until complete. Set to false to use this
    //!   function to kick off a format unit operation for you.
    //!
    //  Exit:
    ...
    M_NONNULL_PARAM_LIST(1)
    M_PARAM_RO(1)
    OPENSEA_OPERATIONS_API eReturnValues run_Format_Unit(const tDevice*          device,
    ...
    //!
    //  Exit:
    //!   \return SUCCESS = format unit was successful, IN_PROGRESS = format unit in progress, !SUCCESS = check error
    //!   code.
    //
    //-----------------------------------------------------------------------------
    M_NONNULL_PARAM_LIST(1)
    M_PARAM_RO(1) OPENSEA_OPERATIONS_API eReturnValues show_Format_Unit_Progress(const tDevice* device);

    typedef struct s_formatStatus
    {
        bool formatParametersAllFs; // This means that the last format failed, or the drive is new, or the data is not
                                    // available right now
    ...
    //  Entry:
    //!   \param[in] device = file descriptor
    //!   \param[out] formatStatus = pointer to structure to hold all the information about the last format command that
    //!   was run and logged by the device.
    //  Exit:
    ...
    M_PARAM_RO(1)
    M_PARAM_WO(2)
    OPENSEA_OPERATIONS_API eReturnValues get_Format_Status(const tDevice* device, ptrFormatStatus formatStatus);
    ...
    //
    //  Entry:
    //!   \param[in] formatStatus = pointer to structure that holds all the information about the last format command
    //!   that was run and logged by the device.
    //  Exit:
    //
    //-----------------------------------------------------------------------------
    M_NONNULL_PARAM_LIST(1)
    ...
    //-----------------------------------------------------------------------------
    M_NONNULL_PARAM_LIST(1)
    M_PARAM_RO(1) OPENSEA_OPERATIONS_API bool is_Set_Sector_Configuration_Supported(const tDevice* device);
    ...
    //!                       This will erase the first and last sector of the drive to ensure that weird issues due to
    //!                       a dummy MBR do not occur for the user.
    //
    //  Entry:
    //!   \param[in] device = file descriptor
    ...
    M_NONNULL_PARAM_LIST(1)
    M_PARAM_RO(1)
    OPENSEA_OPERATIONS_API eReturnValues set_Sector_Configuration(const tDevice* device, uint32_t sectorSize);

    M_NONNULL_PARAM_LIST(1)
    M_PARAM_RO(1)
    OPENSEA_OPERATIONS_API eReturnValues set_Sector_Configuration_With_Force(const tDevice* device,
                                                                             uint32_t       sectorSize,
                                                                             bool           force);
    ...
    {
        // from inquiry
        bool deviceSupportsProtection; // Must be true for remaining data to be valid. If false, all below this are to
                                       // be considered false
        // from extended inquiry VPD page
        bool protectionReportedPerSectorSize; // Set to true if a SCSI device supports the supported logical block
                                              // lengths and protection types VPD page.
    ...
    typedef enum eSectorSizeAddInfoTypeEnum
    {
    ...
    typedef struct s_sectorSize
    {
        ...
        union
        {
            ...
            {
                bool piSupportBitsValid; // If true, the fields below are valid, otherwise they were not reported on a
                                         // sector size basis.
                bool p_i_i_sup;
                bool no_pi_chk;
                bool grd_chk;
            ...
            struct // these aren't directly related to a specific sector size in NVMe, but are important for formatting
                   // information.
            {
    ...
    typedef struct s_supportedFormats
    {
        bool deviceSupportsOtherFormats; // if this is false, then
        bool scsiInformationNotReported; // This flag means, that on a SCSI device, we couldn't get the list of
                                         // supported sector sizes, so the list includes common known sizes and it most
                                         // likely a guesstimate. In otherwords, check the product manual
        bool              scsiFastFormatSupported;
        protectionSupport protectionInformationSupported;
        struct nvmMetaDataModes
        {
...
#define MAX_SECTOR_SIZES_ARRAY (64)   // Based on max from NVMe
    ...
    //!                          formats = malloc(sizeof(ptrSupportedFormats) + numberOfSectorSizes *
    //!                          sizeof(supportedFormats)); formats->numberOfSectorSizes = numberOfSectorSizes;
    //  Entry:
    //!   \param[in] device = file descriptor
    ...
    M_PARAM_RO(1)
    M_PARAM_WO(2)
    OPENSEA_OPERATIONS_API eReturnValues get_Supported_Formats(const tDevice* device, ptrSupportedFormats formats);
    ...
    //! on ATA to set the correct parameters to use in the set sector configuration command.
    //
    //  Entry:
    //!   \param[in] device = file descriptor
    //!   \param[in] logicalBlockLength = the logical block length you want to change to
    //!   \param[out] descriptorCheckCode = pointer to a word that will hold the descriptor check code to send to the
    //!   device \param[out] descriptorIndex = pointer to a byte that will hold the descriptor index to send to the
    //!   device.
    //!
    //  Exit:
    //!   \return SUCCESS = successfully mapped sector size to a descriptor from the device, !SUCCESS = check error
    //!   code.
    //
    //-----------------------------------------------------------------------------
    ...
    M_PARAM_WO(3)
    M_PARAM_WO(4)
    OPENSEA_OPERATIONS_API eReturnValues ata_Map_Sector_Size_To_Descriptor_Check(const tDevice* device,
                                                                                 uint32_t       logicalBlockLength,
    ...
    //-----------------------------------------------------------------------------
    //
    //  get_NVMe_Format_Support(const tDevice * device, ptrNvmeFormatSupport formatSupport)
    //
    ...
    M_PARAM_RO(1)
    M_PARAM_WO(2)
    OPENSEA_OPERATIONS_API eReturnValues get_NVMe_Format_Support(const tDevice*       device,
                                                                 ptrNvmeFormatSupport formatSupport);
    ...
    typedef struct s_nvmFmtPILocation
    {
        bool valid; // if this is not set, then it is assumed to use the last 8 bytes of metadata (PIL bit cleared to
                    // zero) or whatever the device supports
        bool first8Bytes; // set to true for PI data in first 8 bytes of metadata, false for last 8 bytes.
    } nvmFmtPILocation;

    typedef struct s_nvmFmtMetadataSettings
    {
        bool valid; // if this is not set, then it is assumed to use a separate metadata buffer. (or whatever the device
                    // supports)
        bool metadataAsExtendedLBA; // Set to true for metadata to be transferred as part of an extended data LBA. False
                                    // to transfer it as a separate buffer.
    } nvmFmtMetadataSettings;

    typedef struct s_nvmFmtSize
    {
        bool     currentBlockSize; // will reuse the same LBA format that it is already formatted as.
        uint32_t newBlockSize;
        bool     changeMetadataSize; // if true, then the next parameter will be analyzed, otherwise a compatible sector
                                     // size with the specified metadata size will be chosen
        uint16_t metadataSize;
    } nvmFmtSize;

    typedef struct s_runNVMFormatParameters
    {
        bool currentNamespace; // This will only work if the controller supports formatting only the current namespace.
                               // Check FNA bits
        bool formatNumberProvided; // set tot true when specifying one of the 16 formats a NVMe device reports in it's
                                   // identify namespace data. If false, then the sector size will be mapped as best it
                                   // can with other inputs
        union
        {
            nvmFmtSize newSize;   // NOTE: NVMe can report the same sector size with differing metadata sizes so need to
                                  // map incoming size to
    ...
    //-----------------------------------------------------------------------------
    //
    //  run_NVMe_Format(const tDevice * device, runNVMFormatParameters nvmParams, bool pollForProgress)
    //
    ...
    //!   \param[in] pollForProgress = set to true for this function to poll for progress until complete, false to exit
    ...
    M_NONNULL_PARAM_LIST(1)
    M_PARAM_RO(1)
    OPENSEA_OPERATIONS_API eReturnValues run_NVMe_Format(const tDevice*         device,
    ...
    M_PARAM_RO(1)
    M_PARAM_WO(2)
    OPENSEA_OPERATIONS_API eReturnValues get_NVM_Format_Progress(const tDevice* device, uint8_t* percentComplete);
    ...
    //-----------------------------------------------------------------------------
    M_NONNULL_PARAM_LIST(1)
    M_PARAM_RO(1) OPENSEA_OPERATIONS_API eReturnValues show_NVM_Format_Progress(const tDevice* device);
